...
static void* sender(void* arg) {
    while (!brpc::IsAskedToQuit()) {
        ...
        if (braft::rtb::select_leader(FLAGS_group, &leader) != 0) {
            ...
            // by sending RPCs.
            sgxbutil::Status st = braft::rtb::refresh_leader(
                        FLAGS_group, FLAGS_timeout_ms);
            if (!st.ok()) {
                // Not sure about the leader, sleep for a while and then ask again.
        ...
        if (sgxbutil::fast_rand_less_than(100) < (size_t)FLAGS_add_percentage) {
