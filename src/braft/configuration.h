...
struct PeerId {
    sgxbutil::EndPoint addr; // ip+port.
    int idx; // idx in same addr, default 0

    PeerId() : idx(0) {}
    explicit PeerId(sgxbutil::EndPoint addr_) : addr(addr_), idx(0) {}
    PeerId(sgxbutil::EndPoint addr_, int idx_) : addr(addr_), idx(idx_) {}
    /*intended implicit*/PeerId(const std::string& str) 
    { CHECK_EQ(0, parse(str)); }
    PeerId(const PeerId& id) : addr(id.addr), idx(id.idx) {}

    void reset() {
        addr.ip = sgxbutil::IP_ANY;
        addr.port = 0;
        idx = 0;
    }

    bool is_empty() const {
        return (addr.ip == sgxbutil::IP_ANY && addr.port == 0 && idx == 0);
    }
    ...
    int parse(const std::string& str) {
        ...
        if (0 != sgxbutil::str2ip(ip_str, &addr.ip)) {
            reset();
            return -1;
        }
        return 0;
    ...
    std::string to_string() const {
        char str[128];
        snprintf(str, sizeof(str), "%s:%d", sgxbutil::endpoint2str(addr).c_str(), idx);
...
public:
    ...
    int parse_from(sgxbutil::StringPiece conf);
