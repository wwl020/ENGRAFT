...
SnapshotExecutor::~SnapshotExecutor() {
    ...
    CHECK(!_downloading_snapshot.load(sgxbutil::memory_order_relaxed));
...
void SnapshotExecutor::do_snapshot(Closure* done) {
    ...
    if (_downloading_snapshot.load(sgxbutil::memory_order_relaxed)) {
        lck.unlock();
        if (done) {
...
int SnapshotExecutor::on_snapshot_save_done(
    const sgxbutil::Status& st, const SnapshotMeta& meta, SnapshotWriter* writer) {
    std::unique_lock<raft_mutex_t> lck(_mutex);
...
void SnapshotExecutor::on_snapshot_load_done(const sgxbutil::Status& st) {
    std::unique_lock<raft_mutex_t> lck(_mutex);

    CHECK(_loading_snapshot);
    DownloadingSnapshot* m = _downloading_snapshot.load(sgxbutil::memory_order_relaxed);
    ...
    lck.lock();
    _loading_snapshot = false;
    _downloading_snapshot.store(NULL, sgxbutil::memory_order_release);
    lck.unlock();
    if (m) {
        // Respond RPC
        if (!st.ok()) {
...
void SnapshotExecutor::load_downloading_snapshot(DownloadingSnapshot* ds,
    ...
    std::unique_lock<raft_mutex_t> lck(_mutex);
    CHECK_EQ(ds, _downloading_snapshot.load(sgxbutil::memory_order_relaxed));
    ...
    if (!_cur_copier->ok()) {
        ...
        _cur_copier = NULL;
        _downloading_snapshot.store(NULL, sgxbutil::memory_order_relaxed);
...
int SnapshotExecutor::register_downloading_snapshot(DownloadingSnapshot* ds) {
    ...
    if (!m) {
        _downloading_snapshot.store(ds, sgxbutil::memory_order_relaxed);
...
void SnapshotExecutor::interrupt_downloading_snapshot(int64_t new_term) {
    ...
    ss << "Trying to cancel downloading snapshot : " 
       << _downloading_snapshot.load(sgxbutil::memory_order_relaxed)
...
void SnapshotExecutor::describe(std::ostream&os, bool use_html) {
    ...
    const DownloadingSnapshot* m = 
            _downloading_snapshot.load(sgxbutil::memory_order_acquire);
    if (m) {
