...
int BallotBox::commit_at(
    ...
    _last_committed_index.store(last_committed_index, sgxbutil::memory_order_relaxed);
    lck.unlock();
...
int BallotBox::set_last_committed_index(int64_t last_committed_index) {
    ...
    if (last_committed_index < 
            _last_committed_index.load(sgxbutil::memory_order_relaxed)) {
        return EINVAL;
    }
    if (last_committed_index > _last_committed_index.load(sgxbutil::memory_order_relaxed)) {
        _last_committed_index.store(last_committed_index, sgxbutil::memory_order_relaxed);
        lck.unlock();
        _waiter->on_committed(last_committed_index);
    }
