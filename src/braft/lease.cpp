...
void LeaderLease::get_lease_info(LeaseInfo* lease_info) {
    ...
    if (sgxbutil::monotonic_time_ms() < _last_active_timestamp + _election_timeout_ms) {
...
void FollowerLease::init(int64_t election_timeout_ms, int64_t max_clock_drift_ms) {
    ...
    _last_leader_timestamp = sgxbutil::monotonic_time_ms();
}
...
bool FollowerLease::expired() {
    return sgxbutil::monotonic_time_ms() - _last_leader_timestamp
