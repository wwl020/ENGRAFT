...
struct LogEntry : public sgxbutil::RefCountedThreadSafe<LogEntry> {
public:
...
private:
    ...
    friend class sgxbutil::RefCountedThreadSafe<LogEntry>;
    virtual ~LogEntry();
};
...
struct LogIdHasher {
    size_t operator()(const LogId& id) const {
        return sgxbutil::fmix64(id.index) ^ sgxbutil::fmix64(id.term);
    }
};
...
sgxbutil::Status parse_configuration_meta(const sgxbutil::IOBuf& data, LogEntry* entry);

sgxbutil::Status serialize_configuration_meta(const LogEntry* entry, sgxbutil::IOBuf& data);
