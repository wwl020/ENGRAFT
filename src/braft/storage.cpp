...
// DEFINE_bool(raft_sync, false, "call fsync when need");
...
LogStorage* LogStorage::create(const std::string& uri) {
    sgxbutil::StringPiece copied_uri(uri);
    std::string parameter;
    sgxbutil::StringPiece protocol = parse_uri(&copied_uri, &parameter);
    if (protocol.empty()) {
...
sgxbutil::Status LogStorage::destroy(const std::string& uri) {
...
sgxbutil::Status SnapshotStorage::destroy(const std::string& uri) {
    sgxbutil::Status status;
    sgxbutil::StringPiece copied_uri(uri);
    std::string parameter;
    sgxbutil::StringPiece protocol = parse_uri(&copied_uri, &parameter);
...
sgxbutil::Status RaftMetaStorage::destroy(const std::string& uri,
