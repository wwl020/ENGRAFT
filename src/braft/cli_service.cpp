...
static void add_peer_returned(brpc::Controller* cntl,
                          ...
                          sgxscoped_refptr<NodeImpl> /*node*/,
                          ::google::protobuf::Closure* done,
                          const sgxbutil::Status& st) {
...
void CliServiceImpl::add_peer(::google::protobuf::RpcController* controller,
    ...
    brpc::ClosureGuard done_guard(done);
    sgxscoped_refptr<NodeImpl> node;
    sgxbutil::Status st = get_node(&node, request->group_id(), request->leader_id());
    if (!st.ok()) {
        cntl->SetFailed(st.error_code(), "%s", st.error_cstr());
        return;
    }
...
void CliServiceImpl::reset_peer(::google::protobuf::RpcController* controller,
    ...
    brpc::ClosureGuard done_guard(done);
    sgxscoped_refptr<NodeImpl> node;
    sgxbutil::Status st = get_node(&node, request->group_id(), request->peer_id());
    if (!st.ok()) {
        cntl->SetFailed(st.error_code(), "%s", st.error_cstr());
        return;
    }
...
sgxbutil::Status CliServiceImpl::get_node(sgxscoped_refptr<NodeImpl>* node,
                                      const GroupId& group_id,
                                      const std::string& peer_id) {
    if (!peer_id.empty()) {
        *node = global_node_manager->get(group_id, peer_id);
        if (!(*node)) {
            return sgxbutil::Status(ENOENT, "Fail to find node %s in group %s",
                                         peer_id.c_str(),
                                         group_id.c_str());
        }
    } else {
        ...
        if (nodes.empty()) {
            return sgxbutil::Status(ENOENT, "Fail to find node in group %s",
                                         group_id.c_str());
        }
        if (nodes.size() > 1) {
            return sgxbutil::Status(EINVAL, "peer must be specified "
    ...
    if ((*node)->disable_cli()) {
        return sgxbutil::Status(EACCES, "CliService is not allowed to access node "
