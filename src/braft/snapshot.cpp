...
int LocalSnapshotMetaTable::save_to_iobuf_as_remote(sgxbutil::IOBuf* buf) const {
    LocalSnapshotPbMeta pb_meta;
    if (_meta.IsInitialized()) {
        *pb_meta.mutable_meta() = _meta;
    }
    ...
    buf->clear();
    sgxbutil::IOBufAsZeroCopyOutputStream wrapper(buf);
    return pb_meta.SerializeToZeroCopyStream(&wrapper) ? 0 : -1;
}

int LocalSnapshotMetaTable::load_from_iobuf_as_remote(const sgxbutil::IOBuf& buf) {
    LocalSnapshotPbMeta pb_meta;
    sgxbutil::IOBufAsZeroCopyInputStream wrapper(buf);
...
public:
    ...
    int read_file(sgxbutil::IOBuf* out,
                  const std::string &filename,
                  off_t offset,
                  size_t max_count,
                  bool read_partly,
        ...
        if (_snapshot_throttle && FLAGS_raft_enable_throttle_when_install_snapshot) {
            int ret = 0;
            int64_t start = sgxbutil::cpuwide_time_us();
            ...
            if ((ret == 0 || ret == EAGAIN) && used_count < (int64_t)new_max_count) {
                _snapshot_throttle->return_unused_throughput(
                        new_max_count, used_count, sgxbutil::cpuwide_time_us() - start);
            }
            return ret;
        }
...
int LocalSnapshotStorage::init() {
    ...
    if (snapshots.size() > 0) {
        ...
        for (size_t i = 0; i < snapshot_count - 1; i++) {
            ...
            sgxbutil::string_appendf(&snapshot_path, "/" BRAFT_SNAPSHOT_PATTERN, index);
...
void LocalSnapshotStorage::unref(const int64_t index) {
    ...
    if (it != _ref_map.end()) {
        ...
        if (it->second == 0) {
            ...
            sgxbutil::string_appendf(&old_path, "/" BRAFT_SNAPSHOT_PATTERN, index);
...
int LocalSnapshotStorage::close(SnapshotWriter* writer_base,
    ...
    do {
        ...
        sgxbutil::string_appendf(&new_path, "/" BRAFT_SNAPSHOT_PATTERN, new_index);
...
SnapshotReader* LocalSnapshotStorage::open() {
    ...
    if (_last_snapshot_index != 0) {
        ...
        lck.unlock();
        std::string snapshot_path(_path);
        sgxbutil::string_appendf(&snapshot_path, "/" BRAFT_SNAPSHOT_PATTERN, last_snapshot_index);
...
sgxbutil::Status LocalSnapshotStorage::gc_instance(const std::string& uri) const {
...
void LocalSnapshotCopier::load_meta_table() {
    sgxbutil::IOBuf meta_buf;
    std::unique_lock<raft_mutex_t> lck(_mutex);
    if (_cancelled) {
        set_error(ECANCELED, "%s", berror(ECANCELED));
        return;
    }
    sgxscoped_refptr<RemoteFileCopier::Session> session
...
void LocalSnapshotCopier::copy_file(const std::string& filename) {
    ...
    sgxbutil::FilePath sub_path(filename);
    if (sub_path != sub_path.DirName() && sub_path.DirName().value() != ".") {
        sgxbutil::File::Error e;
        bool rc = false;
        if (FLAGS_raft_create_parent_directories) {
            sgxbutil::FilePath sub_dir =
                    sgxbutil::FilePath(_writer->get_path()).Append(sub_path.DirName());
