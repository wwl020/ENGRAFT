...
public:
    IOMetric() 
        : start_time_us(sgxbutil::cpuwide_time_us())
...
inline sgxbutil::StringPiece parse_uri(sgxbutil::StringPiece* uri, std::string* parameter) {
    // ${protocol}://${parameters}
    size_t pos = uri->find("://");
    if (pos == sgxbutil::StringPiece::npos) {
        return sgxbutil::StringPiece();
    }
    sgxbutil::StringPiece protocol = uri->substr(0, pos);
    uri->remove_prefix(pos + 3/* length of '://' */);
    protocol.trim_spaces();
    parameter->reserve(uri->size());
    parameter->clear();
    size_t removed_spaces = 0;
    for (sgxbutil::StringPiece::const_iterator 
...
inline int gc_dir(const std::string& path) {
    ...
    sgxbutil::FilePath target_path(path);
    sgxbutil::FilePath tmp_path(path + ".tmp");
    // delete tmp path firstly in case there is garbage
    if (!sgxbutil::DeleteFile(tmp_path, true)) {
        LOG(ERROR) << "Fail to delete tmp file, path: " << tmp_path.value();
        return -1;
    }

    if (sgxbutil::PathExists(target_path)) {
        const bool rc = sgxbutil::ReplaceFile(sgxbutil::FilePath(target_path),
                                          sgxbutil::FilePath(tmp_path), &e);
        if (!rc) {
...
public:
    ...
    // in |uri|
    virtual sgxbutil::Status gc_instance(const std::string& uri) const {
    ...
    static sgxbutil::Status destroy(const std::string& uri);
};
...
public:
    ...
    // init stable storage
    virtual sgxbutil::Status init() = 0;
    ...
    // in |uri|
    virtual sgxbutil::Status gc_instance(const std::string& uri, 
...
// Snapshot 
class Snapshot : public sgxbutil::Status {
public:
    Snapshot() {}
    virtual ~Snapshot() {}
...
class SnapshotCopier : public sgxbutil::Status {
public:
