...
template <typename T> class sgxscoped_refptr;
...
const PeerId ANY_PEER(sgxbutil::EndPoint(sgxbutil::IP_ANY, 0), 0);

// Raft-specific closure which encloses a sgxbutil::Status to report if the
// operation was successful.
class Closure : public google::protobuf::Closure {
public:
    sgxbutil::Status& status() { return _st; }
...
public:
    ...
    const sgxbutil::IOBuf& data() const;
    ...
    void set_error_and_rollback(size_t ntail = 1, const sgxbutil::Status* st = NULL);
...
public:
    ...
    virtual void on_leader_stop(const sgxbutil::Status& status);
...
public:
    LeaderChangeContext(const PeerId& leader_id, int64_t term, const sgxbutil::Status& status)
...
public:
    UserLog() {};
    UserLog(int64_t log_index, const sgxbutil::IOBuf& log_data)
        : _index(log_index)
        , _data(log_data)
    {};
    int64_t log_index() const { return _index; }
    const sgxbutil::IOBuf& log_data() const { return _data; }
    void set_log_index(const int64_t log_index) { _index = log_index; }
    void set_log_data(const sgxbutil::IOBuf& log_data) { _data = log_data; }
    void reset() {
        _index = 0;
        _data.clear();
    }
...
struct NodeOptions {
    ...
    // Default: NULL
    sgxscoped_refptr<FileSystemAdaptor>* snapshot_file_system_adaptor;    
    
    // If non-null, we will pass this snapshot_throttle to SnapshotExecutor
    // Default: NULL
    sgxscoped_refptr<SnapshotThrottle>* snapshot_throttle;
...
public:
    ...
    sgxbutil::Status list_peers(std::vector<PeerId>* peers);
    ...
    sgxbutil::Status reset_peers(const Configuration& new_peers);
    ...
    // higher probability
    sgxbutil::Status vote(int election_timeout);
    ...
    // unchanged.
    sgxbutil::Status reset_election_timeout_ms(int election_timeout_ms);
    ...
    sgxbutil::Status read_committed_user_log(const int64_t index, UserLog* user_log);
...
// Returns 0 on success, -1 otherwise.
int add_service(brpc::Server* server, const sgxbutil::EndPoint listen_addr);
