...
void FileServiceImpl::get_file(::google::protobuf::RpcController* controller,
    ...
    sgxscoped_refptr<FileReader> reader;
    ...
    if (!FLAGS_raft_file_check_hole) {
        ...
        while (!buf.empty()) {
            sgxbutil::StringPiece p = buf.backing_block(0);
            if (!is_zero(p.data(), p.size())) {
                sgxbutil::IOBuf piece_buf;
...
FileServiceImpl::FileServiceImpl() {
    _next_id = ((int64_t)getpid() << 45) | (sgxbutil::gettimeofday_us() << 17 >> 17);
}
