...
// bvar::Adder<int64_t> g_nentries("raft_num_log_entries"); //- Remove bvar

LogEntry::LogEntry(): type(ENTRY_TYPE_UNKNOWN), peers(NULL), old_peers(NULL) {
    // g_nentries << 1; //- Remove bvar
}
...
sgxbutil::Status parse_configuration_meta(const sgxbutil::IOBuf& data, LogEntry* entry) {
    ...
    sgxbutil::IOBufAsZeroCopyInputStream wrapper(data);
...
sgxbutil::Status serialize_configuration_meta(const LogEntry* entry, sgxbutil::IOBuf& data) {
    ...
    sgxbutil::IOBufAsZeroCopyOutputStream wrapper(&data);
