...
class FileReader : public sgxbutil::RefCountedThreadSafe<FileReader> {
friend class sgxbutil::RefCountedThreadSafe<FileReader>;
public:
    ...
    virtual int read_file(sgxbutil::IOBuf* out,
...
protected:
    int read_file_with_meta(sgxbutil::IOBuf* out,
                            const std::string &filename,
                            ...
                            size_t* read_count,
                            bool* is_eof) const;
    const sgxscoped_refptr<FileSystemAdaptor>& file_system() const { return _fs; }

private:
    std::string _path;
    sgxscoped_refptr<FileSystemAdaptor> _fs;
};
