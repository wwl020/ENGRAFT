...
int add_service(brpc::Server* server, int port) {
    sgxbutil::EndPoint addr(sgxbutil::IP_ANY, port);
    return add_service(server, addr);
}

int add_service(brpc::Server* server, const char* listen_ip_and_port) {
    sgxbutil::EndPoint addr;
    if (sgxbutil::str2endpoint(listen_ip_and_port, &addr) != 0) {
...
int gc_raft_data(const GCOptions& gc_options) {
    ...
    sgxbutil::Status status = LogStorage::destroy(log_uri);
    if (!status.ok()) {
        is_success = false;
...
sgxbutil::Status Node::list_peers(std::vector<PeerId>* peers) {
...
sgxbutil::Status Node::reset_peers(const Configuration& new_peers) {
...
sgxbutil::Status Node::vote(int election_timeout) {
    return _impl->vote(election_timeout);
}

sgxbutil::Status Node::reset_election_timeout_ms(int election_timeout_ms) {
...
sgxbutil::Status Node::read_committed_user_log(const int64_t index, UserLog* user_log) {
...
const sgxbutil::IOBuf& Iterator::data() const {
...
void Iterator::set_error_and_rollback(size_t ntail, const sgxbutil::Status* st) {
...
void StateMachine::on_snapshot_save(SnapshotWriter* writer, Closure* done) {
    ...
    LOG(ERROR) << "Statemachine didn't implement on_snapshot_save";
    // done->status().set_error(-1, "%s didn't implement on_snapshot_save",
    //                              butil::class_name_str(*this).c_str());
    done->status().set_error(-1, "Statemachine didn't implement on_snapshot_save");                                 
    done->Run();
}
...
void StateMachine::on_leader_stop(const sgxbutil::Status&) {}
