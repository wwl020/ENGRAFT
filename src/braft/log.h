...
class BAIDU_CACHELINE_ALIGNMENT Segment 
        : public sgxbutil::RefCountedThreadSafe<Segment> {
public:
...
private:
friend class sgxbutil::RefCountedThreadSafe<Segment>;
    ~Segment() {
        if (_fd >= 0) {
            ::close(_fd);
            _fd = -1;
...
public:
    typedef std::map<int64_t, sgxscoped_refptr<Segment> > SegmentMap;
    ...
    void sync();
private:
    sgxscoped_refptr<Segment> open_segment();
    ...
    int get_segment(int64_t log_index, sgxscoped_refptr<Segment>* ptr);
    void pop_segments(
            int64_t first_index_kept, 
            std::vector<sgxscoped_refptr<Segment> >* poped);
    ...
    sgxscoped_refptr<Segment> _open_segment;
    int _checksum_type;
    bool _enable_sync;
};
