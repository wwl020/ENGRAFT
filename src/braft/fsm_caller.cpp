...
//         "raft_commit_tasks_batch_counter"); //- Remove bvar
    ...
    for (; iter; ++iter) {
        if (iter->type == COMMITTED) {
            ...
            if (max_committed_index >= 0) {
                ...
                // g_commit_tasks_batch_counter << counter; //- Remove bvar
                counter = 0;
            }
            switch (iter->type) {
            case COMMITTED:
...
void FSMCaller::do_committed(int64_t committed_index) {
    ...
    _last_applied_index.store(committed_index, sgxbutil::memory_order_release);
...
void FSMCaller::do_snapshot_save(SaveSnapshotClosure* done) {
    ...
    int64_t last_applied_index = _last_applied_index.load(sgxbutil::memory_order_relaxed);
...
void FSMCaller::do_snapshot_load(LoadSnapshotClosure* done) {
    ...
    last_applied_id.index = _last_applied_index.load(sgxbutil::memory_order_relaxed);
...
int FSMCaller::on_leader_stop(const sgxbutil::Status& status) {
    ApplyTask task;
    task.type = LEADER_STOP;
    sgxbutil::Status* on_leader_stop_status = new sgxbutil::Status(status);
...
void FSMCaller::do_leader_stop(const sgxbutil::Status& status) {
...
int64_t FSMCaller::applying_index() const {
    ...
    if (cur_task != COMMITTED) {
        ...
        return _applying_index.load(sgxbutil::memory_order_relaxed);
    }
}
...
IteratorImpl::IteratorImpl(StateMachine* sm, LogManager* lm,
                          ...
                          sgxbutil::atomic<int64_t>* applying_index)
        : _sm(sm)
        , _lm(lm)
        , _closure(closure)
...
void IteratorImpl::next() {
    ...
    if (_cur_index <= _committed_index) {
        ...
        if (_cur_index <= _committed_index) {
            ...
            _applying_index->store(_cur_index, sgxbutil::memory_order_relaxed);
        }
    }
}
...
void IteratorImpl::set_error_and_rollback(
            size_t ntail, const sgxbutil::Status* st) {
    if (ntail == 0) {
