...
int EncryptedSCSegment::_load_entry(off_t offset, EntryHeader* head, 
                                sgxbutil::IOBuf* data, size_t size_hint) const {
...
int EncryptedSCSegment::_extract_log_entry_header(sgxbutil::IOBuf read_iobuf, unsigned char* iv, unsigned char* aad, unsigned char* mac, EntryHeader* head) const {
    size_t buf_offset = 0;
    //- 1. Extract iv
...
int EncryptedSCSegment::_load_whole_entry(off_t offset, EntryHeader* head, sgxbutil::IOBuf* data,
...
int EncryptedSCSegment::append(const LogEntry* entry) {
    ...
    if (sgxbutil::generate_sha256_hash(data_plain, data_bytes, temp_log_hash_sum.log_hash) != 0) {
    ...
    if (sgxbutil::generate_sha256_hash(two_hash, 64, temp_log_hash_sum.log_hash_sum) != 0) {
...
int EncryptedSCSegmentLogStorage::append_entries(const std::vector<LogEntry*>& entries, IOMetric* metric) {
    ...
    sgxscoped_refptr<EncryptedSCSegment> last_segment = NULL;
    int64_t now = 0;
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        sgxscoped_refptr<EncryptedSCSegment> segment = open_segment();
...
LogEntry* EncryptedSCSegmentLogStorage::get_entry(const int64_t index) {
    sgxscoped_refptr<EncryptedSCSegment> ptr;
...
void EncryptedSCSegmentLogStorage::pop_segments(
        ...
        std::vector<sgxscoped_refptr<EncryptedSCSegment> >* popped) {
    popped->clear();
    popped->reserve(32);
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    for (SegmentMap::iterator it = _segments.begin(); it != _segments.end();) {
        sgxscoped_refptr<EncryptedSCSegment>& segment = it->second;
...
sgxscoped_refptr<EncryptedSCSegment> EncryptedSCSegmentLogStorage::open_segment() {
    // sgxscoped_refptr<EncryptedSCSegment> prev_open_segment;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        if (!_open_segment) {
...
int EncryptedSCSegmentLogStorage::get_segment(int64_t index, sgxscoped_refptr<EncryptedSCSegment>* ptr) {
    BAIDU_SCOPED_LOCK(_mutex);
...
sgxbutil::Status EncryptedSCSegmentLogStorage::gc_instance(const std::string& uri) const {
