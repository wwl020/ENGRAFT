...
bool NodeManager::server_exists(sgxbutil::EndPoint addr) {
    BAIDU_SCOPED_LOCK(_mutex);
    if (addr.ip != sgxbutil::IP_ANY) {
        sgxbutil::EndPoint any_addr(sgxbutil::IP_ANY, addr.port);
...
void NodeManager::remove_address(sgxbutil::EndPoint addr) {
    BAIDU_SCOPED_LOCK(_mutex);
    _addr_set.erase(addr);
}

int NodeManager::add_service(brpc::Server* server, 
                             const sgxbutil::EndPoint listen_address) {
    if (server == NULL) {
...
sgxscoped_refptr<NodeImpl> NodeManager::get(const GroupId& group_id, const PeerId& peer_id) {
    sgxbutil::DoublyBufferedData<Maps>::ScopedPtr ptr;
...
void NodeManager::get_nodes_by_group_id(
        const GroupId& group_id, std::vector<sgxscoped_refptr<NodeImpl> >* nodes) {
...
void NodeManager::get_all_nodes(std::vector<sgxscoped_refptr<NodeImpl> >* nodes) {
    nodes->clear();
    sgxbutil::DoublyBufferedData<Maps>::ScopedPtr ptr;
    if (_nodes.Read(&ptr) != 0) {
        return;
