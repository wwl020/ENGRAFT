...
int RemoteFileCopier::init(const std::string& uri, FileSystemAdaptor* fs, 
    ...
    sgxbutil::StringPiece uri_str(uri);
    ...
    sgxbutil::StringPiece ip_and_port = uri_str.substr(0, slash_pos);
    uri_str.remove_prefix(slash_pos + 1);
    if (!sgxbutil::StringToInt64(uri_str, &_reader_id)) {
...
int RemoteFileCopier::copy_to_file(const std::string& source,
    ...
    sgxscoped_refptr<Session> session = start_to_copy_to_file(
...
int RemoteFileCopier::copy_to_iobuf(const std::string& source,
                                    sgxbutil::IOBuf* dest_buf, 
                                    const CopyOptions* options) {
    sgxscoped_refptr<Session> session = start_to_copy_to_iobuf(
...
sgxscoped_refptr<RemoteFileCopier::Session> 
RemoteFileCopier::start_to_copy_to_file(
    ...
    sgxscoped_refptr<Session> session(new Session());
...
void RemoteFileCopier::Session::send_next_rpc() {
    ...
    if (_throttle && FLAGS_raft_enable_throttle_when_install_snapshot) {
        _throttle_token_acquire_time_us = sgxbutil::cpuwide_time_us();
        ...
        if (new_max_count == 0) {
            ...
            if (bthread_timer_add(
                    &_timer, 
                    sgxbutil::milliseconds_from_now(retry_interval_ms_when_throttled),
                    on_timer, this) != 0) {
                lck.unlock();
...
void RemoteFileCopier::Session::on_rpc_returned() {
    sgxscoped_refptr<Session> ref_gurad;
        ...
        if (_cntl.ErrorCode() == EAGAIN && _throttle) {
            ...
            if (FLAGS_raft_enable_throttle_when_install_snapshot) {
                _throttle->return_unused_throughput(
                        request_count, 0,
                        sgxbutil::cpuwide_time_us() - _throttle_token_acquire_time_us);
            }
        }
        AddRef();
        if (bthread_timer_add(
                    &_timer, 
                    sgxbutil::milliseconds_from_now(retry_interval_ms),
                    on_timer, this) != 0) {
            lck.unlock();
            LOG(ERROR) << "Fail to add timer";
            return on_timer(this);
