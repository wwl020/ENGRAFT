...
LogEntry* MemoryLogStorage::get_entry(const int64_t index) {
    ...
    if (index < _first_log_index.load(sgxbutil::memory_order_relaxed)
            || index > _last_log_index.load(sgxbutil::memory_order_relaxed)) {
        return NULL;
    }
    LogEntry* temp = _log_entry_data[index - _first_log_index.load(sgxbutil::memory_order_relaxed)];
    temp->AddRef();
...
int64_t MemoryLogStorage::get_term(const int64_t index) {
    ...
    LogEntry* temp = _log_entry_data.at(index - _first_log_index.load(sgxbutil::memory_order_relaxed));
...
int MemoryLogStorage::append_entry(const LogEntry* input_entry) {
    ...
    if (input_entry->id.index !=
            _last_log_index.load(sgxbutil::memory_order_relaxed) + 1) {
    ...
    _last_log_index.fetch_add(1, sgxbutil::memory_order_relaxed);
    lck.unlock();
    return 0;
}
...
int MemoryLogStorage::truncate_prefix(const int64_t first_index_kept) {
    ...
    if (_first_log_index.load(sgxbutil::memory_order_relaxed)
        ...
        _last_log_index.store(first_index_kept - 1, sgxbutil::memory_order_release);
    }
    lck.unlock();
...
sgxbutil::Status MemoryLogStorage::gc_instance(const std::string& uri) const {
