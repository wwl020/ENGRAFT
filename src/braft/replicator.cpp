...
#include <memory>
#include "sgxbutil/time.h"                          // sgxbutil::gettimeofday_us
...
int Replicator::start(const ReplicatorOptions& options, ReplicatorId *id) {
    ...
    r->_last_rpc_send_timestamp = sgxbutil::monotonic_time_ms();
    r->_start_heartbeat_timer(sgxbutil::gettimeofday_us());
...
void Replicator::_block(long start_time_us, int error_code) {
    ...
    const timespec due_time = sgxbutil::milliseconds_from(
	    sgxbutil::microseconds_to_timespec(start_time_us), blocking_time);
...
void Replicator::_on_heartbeat_returned(
    ...
    Replicator *r = NULL;
    bthread_id_t dummy_id = { id };
    const long start_time_us = sgxbutil::gettimeofday_us();
    if (bthread_id_lock(dummy_id, (void**)&r) != 0) {
        return;
    }
...
void Replicator::_on_rpc_returned(ReplicatorId id, brpc::Controller* cntl,
    ...
    if (entries_size > 0) {
        ...
        // g_send_entries_latency << cntl->latency_us(); //- Remove bvar
        if (cntl->request_attachment().size() > 0) {
            // g_normalized_send_entries_latency << 
            //     cntl->latency_us() * 1024 / cntl->request_attachment().size(); //- Remove bvar
        }
    }
    ...
    // VLOG(79) << "Func: " << __FUNCTION__ << " Calling r->_send_entries()";
    r->_send_entries();
    return;
}
...
void Replicator::_send_empty_entries(bool is_heartbeat) {
    ...
    google::protobuf::Closure* done = brpc::NewCallback(
                ...
                //- TODO: How about use CLOCK_REALTIME in sgxbutil::monotonic_time_ms?
...
int Replicator::_prepare_entry(int offset, EntryMeta* em, sgxbutil::IOBuf *data) {
    if (data->length() >= (size_t)FLAGS_raft_max_body_size) {
        return ERANGE;
    }
    const int64_t log_index = _next_index + offset;
    // VLOG(79) << "Func: " << __FUNCTION__ << " Calling log_manager->get_entry";
...
void Replicator::_send_entries() {
    ...
    // g_send_entries_batch_counter << request->entries_size(); //- Remove bvar
    ...
    google::protobuf::Closure* done = brpc::NewCallback(
                ...
                request.get(), response.get(), sgxbutil::monotonic_time_ms());
                //- TODO: How about use CLOCK_REALTIME in sgxbutil::monotonic_time_ms?
    RaftService_Stub stub(&_sending_channel);
    stub.append_entries(cntl.release(), request.release(), 
                        response.release(), done);
...
void Replicator::_install_snapshot() {
    ...
    if (_options.snapshot_throttle && !_options.snapshot_throttle->
        ...
        return _block(sgxbutil::gettimeofday_us(), EBUSY);
    }
...
void Replicator::_on_install_snapshot_returned(
    ...
    // dummy_id is unlock in _send_entries
    if (!succ) {
        return r->_block(sgxbutil::gettimeofday_us(), cntl->ErrorCode());
    }
    r->_has_succeeded = true;
    r->_notify_on_caught_up(0, false);
    if (r->_timeout_now_index > 0 && r->_timeout_now_index < r->_min_flying_index()) {
...
void Replicator::_start_heartbeat_timer(long start_time_us) {
    const timespec due_time = sgxbutil::milliseconds_from(
            sgxbutil::microseconds_to_timespec(start_time_us), 
