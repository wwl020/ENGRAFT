...
using ::sgxbutil::RawPacker;
using ::sgxbutil::RawUnpacker;
...
int Segment::create() {
    ...
    sgxbutil::string_appendf(&path, "/" BRAFT_SEGMENT_OPEN_PATTERN, _first_index);
...
inline bool verify_checksum(int checksum_type, 
                            const sgxbutil::IOBuf& data, uint32_t value) {
    switch (checksum_type) {
    case CHECKSUM_MURMURHASH32:
...
inline uint32_t get_checksum(int checksum_type, const sgxbutil::IOBuf& data) {
    switch (checksum_type) {
    case CHECKSUM_MURMURHASH32:
...
int Segment::_load_entry(off_t offset, EntryHeader* head, sgxbutil::IOBuf* data,
                         size_t size_hint) const {
    sgxbutil::IOPortal buf;
...
int Segment::append(const LogEntry* entry) {
    ...
    while (written < (ssize_t)to_write) {
        //- cut_multiple_into_file_descriptor func finally writes data to disk, 
        //- through the file descriptor _fd
...
int Segment::close(bool will_sync) {
    ...
    sgxbutil::string_appendf(&old_path, "/" BRAFT_SEGMENT_OPEN_PATTERN,
                         _first_index);
    std::string new_path(_path);
    sgxbutil::string_appendf(&new_path, "/" BRAFT_SEGMENT_CLOSED_PATTERN, 
                         _first_index, _last_index.load());
...
std::string Segment::file_name() {
    if (!_is_open) {
        return sgxbutil::string_printf(BRAFT_SEGMENT_CLOSED_PATTERN, _first_index, _last_index.load());
    } else {
        return sgxbutil::string_printf(BRAFT_SEGMENT_OPEN_PATTERN, _first_index);
    }
}
...
int Segment::unlink() {
    ...
    do {
        std::string path(_path);
        if (_is_open) {
            sgxbutil::string_appendf(&path, "/" BRAFT_SEGMENT_OPEN_PATTERN,
                                 _first_index);
        } else {
            sgxbutil::string_appendf(&path, "/" BRAFT_SEGMENT_CLOSED_PATTERN,
                                _first_index, _last_index.load());
...
int Segment::truncate(const int64_t last_index_kept) {
    ...
    if (!_is_open) {
        std::string old_path(_path);
        sgxbutil::string_appendf(&old_path, "/" BRAFT_SEGMENT_CLOSED_PATTERN,
                             _first_index, _last_index.load());

        std::string new_path(_path);
        sgxbutil::string_appendf(&new_path, "/" BRAFT_SEGMENT_OPEN_PATTERN,
                             _first_index);
...
int SegmentLogStorage::init(ConfigurationManager* configuration_manager) {
    ...
    if (sgxbutil::crc32c::IsFastCrc32Supported()) {
...
int SegmentLogStorage::append_entries(const std::vector<LogEntry*>& entries, IOMetric* metric) {
    ...
    sgxscoped_refptr<Segment> last_segment = NULL;
    int64_t now = 0;
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        sgxscoped_refptr<Segment> segment = open_segment();
        if (FLAGS_raft_trace_append_entry_latency && metric) {
            ...
            // g_open_segment_latency << delta_time_us; //- Remove bvar
        }
        if (NULL == segment) {
            return i;
        }
        //- Call Segment::append func to conduct appending
        ...
        if (FLAGS_raft_trace_append_entry_latency && metric) {
            ...
            // g_segment_append_entry_latency << delta_time_us; //- Remove bvar
        }
    ...
    if (FLAGS_raft_trace_append_entry_latency && metric) {
        ...
        // g_sync_segment_latency << delta_time_us;  //- Remove bvar
    }
    return entries.size();
}
...
void SegmentLogStorage::pop_segments(
        ...
        std::vector<sgxscoped_refptr<Segment> >* popped) {
    popped->clear();
    popped->reserve(32);
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    for (SegmentMap::iterator it = _segments.begin(); it != _segments.end();) {
        sgxscoped_refptr<Segment>& segment = it->second;
...
void SegmentLogStorage::pop_segments_from_back(
        ...
        std::vector<sgxscoped_refptr<Segment> >* popped,
        sgxscoped_refptr<Segment>* last_segment) {
    popped->clear();
    popped->reserve(32);
    *last_segment = NULL;
    BAIDU_SCOPED_LOCK(_mutex);
    _last_log_index.store(last_index_kept, sgxbutil::memory_order_release);
    if (_open_segment) {
    ...
    if (_segments.rbegin() != _segments.rend()) {
        *last_segment = _segments.rbegin()->second;
        ...
        // next index
        _first_log_index.store(last_index_kept + 1, sgxbutil::memory_order_release);
    }
}
...
int SegmentLogStorage::truncate_suffix(const int64_t last_index_kept) {
    ...
    if (last_segment) {
        if (_first_log_index.load(sgxbutil::memory_order_relaxed) <=
            _last_log_index.load(sgxbutil::memory_order_relaxed)) {
...
int SegmentLogStorage::list_segments(bool is_empty) {
    ...
    for (it = _segments.begin(); it != _segments.end(); ) {
        ...
        if (segment->first_index() > segment->last_index()) {
            ...
            return -1;
                    ...
                    _first_log_index.load(sgxbutil::memory_order_acquire) 
                    < segment->first_index()) {
            LOG(WARNING) << "closed segment has hole, path: " << _path
                << " first_log_index: " << _first_log_index.load(sgxbutil::memory_order_relaxed)
                << " first_index: " << segment->first_index()
                << " last_index: " << segment->last_index();
            return -1;
        } else if (last_log_index == -1 &&
    ...
    if (_open_segment) {
        if (last_log_index == -1 &&
                _first_log_index.load(sgxbutil::memory_order_relaxed) < _open_segment->first_index()) {
...
int SegmentLogStorage::load_segments(ConfigurationManager* configuration_manager) {
    ...
    for (it = _segments.begin(); it != _segments.end(); ++it) {
        ...
        _last_log_index.store(segment->last_index(), sgxbutil::memory_order_release);
    }
...
sgxscoped_refptr<Segment> SegmentLogStorage::open_segment() {
    sgxscoped_refptr<Segment> prev_open_segment;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        if (!_open_segment) {
...
int SegmentLogStorage::get_segment(int64_t index, sgxscoped_refptr<Segment>* ptr) {
    BAIDU_SCOPED_LOCK(_mutex);
...
void SegmentLogStorage::sync() {
    std::vector<sgxscoped_refptr<Segment> > segments;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        for (SegmentMap::iterator it = _segments.begin(); it != _segments.end(); ++it) {
...
sgxbutil::Status SegmentLogStorage::gc_instance(const std::string& uri) const {
