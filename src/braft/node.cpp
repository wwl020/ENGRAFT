...
int SnapshotTimer::adjust_timeout_ms(int timeout_ms) {
    ...
    if (timeout_ms > 0) {
        timeout_ms = sgxbutil::fast_rand_less_than(timeout_ms) + 1;
    }
...
inline int random_timeout(int timeout_ms) {
    ...
    return sgxbutil::fast_rand_in(timeout_ms, timeout_ms + delta);
}
...
NodeImpl::NodeImpl(const GroupId& group_id, const PeerId& peer_id)
    ...
    sgxbutil::string_printf(&_v_group_id, "%s_%d", _group_id.c_str(), _server_id.idx);
    AddRef();
    // g_num_nodes << 1; //- Remove bvar
}
...
int NodeImpl::init_meta_storage() {
    ...
    // check init
    sgxbutil::Status status = _meta_storage->init();
    if (!status.ok()) {
        LOG(WARNING) << "node " << _group_id << ":" << _server_id
...
void NodeImpl::handle_snapshot_timeout() {
    LOG(INFO) << "Func: " << __FUNCTION__  << " pthread-" << pthread_self() << " Concrete-Braft-Timeout";
...
int NodeImpl::bootstrap(const BootstrapOptions& options) {
    ...
    if (_current_term == 0) {
        _current_term = 1;
        sgxbutil::Status status = _meta_storage->
...
int NodeImpl::init(const NodeOptions& options) {
    ...
    // check _server_id
    if (sgxbutil::IP_ANY == _server_id.addr.ip) {
    ...
    sgxbutil::Status st = _log_manager->check_consistency();
    if (!st.ok()) {
        LOG(ERROR) << "node " << _group_id << ":" << _server_id
    ...
    if (!_conf.empty()) {
        step_down(_current_term, false, sgxbutil::Status::OK());
    }
...
void NodeImpl::apply(const Task& task) {
    ...
    LOG(INFO) << "Func: " << __FUNCTION__  << " pthread-" << pthread_self() << " Calling _apply_queue->execute";
...
void NodeImpl::on_caughtup(const PeerId& peer, int64_t term,
                           int64_t version, const sgxbutil::Status& st) {
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    if (st.error_code() == ETIMEDOUT 
        ...
        timespec due_time = sgxbutil::milliseconds_from_now(
                _options.election_timeout_ms);
...
void NodeImpl::handle_stepdown_timeout() {
    ...
    int64_t now = sgxbutil::monotonic_time_ms();
...
sgxbutil::Status NodeImpl::list_peers(std::vector<PeerId>* peers) {
    BAIDU_SCOPED_LOCK(_mutex);
    if (_state != STATE_LEADER) {
        return sgxbutil::Status(EPERM, "Not leader");
    }
...
sgxbutil::Status NodeImpl::reset_peers(const Configuration& new_peers) {
    BAIDU_SCOPED_LOCK(_mutex);

    if (new_peers.empty()) {
        LOG(WARNING) << "node " << _group_id << ":" << _server_id << " set empty peers";
        return sgxbutil::Status(EINVAL, "new_peers is empty");
    }
    // check state
    if (!is_active_state(_state)) {
        LOG(WARNING) << "node " << _group_id << ":" << _server_id
                     << " is in state " << state2str(_state) << ", can't reset_peer";
        return sgxbutil::Status(EPERM, "Bad state %s", state2str(_state));
    }
    // check bootstrap
    if (_conf.conf.empty()) {
        LOG(INFO) << "node " << _group_id << ":" << _server_id 
    ...
    if (_state == STATE_LEADER && _conf_ctx.is_busy()) {
        ...
        return sgxbutil::Status(EBUSY, "Changing to another configuration");
    }
...
void NodeImpl::handle_election_timeout() {
    ...
    // check state
    if (_state != STATE_FOLLOWER) {
        LOG(INFO) << "Func: " << __FUNCTION__  << " pthread-" << pthread_self() << " state is " << State(_state) << ", so return directly.";
        return;
    }

    // Trigger vote manually, or wait until follower lease expire.
    if (!_vote_triggered && !_follower_lease.expired()) {
        LOG(INFO) << "Func: " << __FUNCTION__  << " pthread-" << pthread_self() << " both _vote_triggered and _follower_lease.expired() are false, return directly.";
        return;
    }
    LOG(INFO) << "Func: " << __FUNCTION__  << " pthread-" << pthread_self() << " _vote_triggered is " << _vote_triggered << " _follower_lease.expired() = " << _follower_lease.expired() << ", so continue.";
    _vote_triggered = false;
...
void NodeImpl::handle_timeout_now_request(brpc::Controller* controller,
    ...
    const sgxbutil::EndPoint remote_side = controller->remote_side();
    const int64_t saved_term = _current_term;
...
void on_transfer_timeout(void* arg) {
    LOG(INFO) << "Func: " << __FUNCTION__  << " pthread-" << pthread_self() << " Braft-Timeout";
...
int NodeImpl::transfer_leadership_to(const PeerId& peer) {
    ...
    if (bthread_timer_add(&_transfer_timer,
                       sgxbutil::milliseconds_from_now(_options.election_timeout_ms),
...
sgxbutil::Status NodeImpl::vote(int election_timeout_ms) {
    std::unique_lock<raft_mutex_t> lck(_mutex);
    if (_state != STATE_FOLLOWER) {
        return sgxbutil::Status(EPERM, "is not follower");
    }
    ...
    if (election_timeout_ms > max_election_timeout_ms) {
        return sgxbutil::Status(EINVAL, "election_timeout_ms larger than safety threshold");
    }
    ...
    return sgxbutil::Status();
}

sgxbutil::Status NodeImpl::reset_election_timeout_ms(int election_timeout_ms) {
    std::unique_lock<raft_mutex_t> lck(_mutex);
    int max_election_timeout_ms = _options.max_clock_drift_ms + _options.election_timeout_ms;
    if (election_timeout_ms > max_election_timeout_ms) {
        return sgxbutil::Status(EINVAL, "election_timeout_ms larger than safety threshold");
...
void NodeImpl::become_leader() {
    ...
    // LOG(INFO) << "node " << _group_id << ":" << _server_id
    VLOG(79) << "node " << _group_id << ":" << _server_id
              << " term " << _current_term
...
void LeaderStableClosure::Run() {
    if (status().ok()) {
        ...
        int64_t now = sgxbutil::cpuwide_time_us();
...
void NodeImpl::apply(LogEntryAndClosure tasks[], size_t size) {
    // g_apply_tasks_batch_counter << size; //- Remove bvar
...
int NodeImpl::handle_pre_vote_request(const RequestVoteRequest* request,
    ...
    // if (candidate_id.addr.port != 8100) {
    //     return -1;
    // }
...
int NodeImpl::handle_request_vote_request(const RequestVoteRequest* request,
    ...
    //- TODO: For test only! We need to select 8100 as leader in WAN evaluation
    VLOG(79) << "server_id = " << request->server_id();
    if (candidate_id.addr.port != 8100) {
        return -1;
    }
...
void NodeImpl::handle_append_entries_request(brpc::Controller* cntl,
    ...
    FollowerStableClosure* c = new FollowerStableClosure(
            ...
            this, _current_term);
    // VLOG(79) << "Func: " << __FUNCTION__ << " append entries...";
...
int NodeImpl::increase_term_to(int64_t new_term, const sgxbutil::Status& status) {
    BAIDU_SCOPED_LOCK(_mutex);
...
sgxbutil::Status NodeImpl::read_committed_user_log(const int64_t index, UserLog* user_log) {
    if (index <= 0) {
        return sgxbutil::Status(EINVAL, "request index:%" PRId64 " is invalid.", index);
    }
    const int64_t saved_last_applied_index = _fsm_caller->last_applied_index();
    if (index > saved_last_applied_index) {
        return sgxbutil::Status(ENOMOREUSERLOG, "request index:%" PRId64 " is greater"
    ...
    if (entry == NULL) {
        return sgxbutil::Status(ELOGDELETED, "user log is deleted at index:%" PRId64, index);
    }
    do {
        ...
        if (cur_index > saved_last_applied_index) {
            return sgxbutil::Status(ENOMOREUSERLOG, "no user log between index:%" PRId64
    ...
    return sgxbutil::Status(ELOGDELETED, "user log is deleted at index:%" PRId64, cur_index);
}
...
void NodeImpl::describe(std::ostream& os, bool use_html) {
    ...
    if (st == STATE_FOLLOWER) {
        ...
        os << newline;
        os << "last_msg_to_now: " << sgxbutil::monotonic_time_ms() - leader_timestamp 
           << newline;
    }
...
bool NodeImpl::handle_out_of_order_append_entries(brpc::Controller* cntl,
    ...
    rpc->done = done;
    rpc->receive_time_ms = sgxbutil::gettimeofday_ms();
...
void* NodeImpl::handle_append_entries_from_cache(void* arg) {
    ...
    sgxbutil::LinkedList<AppendEntriesRpc>& rpcs = handle_arg->rpcs;
    while (!rpcs.empty()) {
...
void NodeImpl::AppendEntriesCache::start_to_handle(HandleAppendEntriesFromCacheArg* arg) {
    ...
    if (bthread_start_background(
        ...
        sgxbutil::LinkedList<AppendEntriesRpc>& rpcs = arg->rpcs;
        while (!rpcs.empty()) {
            AppendEntriesRpc* rpc = rpcs.head()->value();
            rpc->RemoveFromList();
            ack_fail(rpc);
...
bool NodeImpl::AppendEntriesCache::start_timer() {
    ...
    timespec duetime = sgxbutil::milliseconds_from(
            sgxbutil::milliseconds_to_timespec(timer_arg->timer_start_ms),
...
void NodeImpl::ConfigurationCtx::on_caughtup(
    ...
    sgxbutil::Status err(ECATCHUP, "Peer %s failed to catch up",
    ...
    case STAGE_STABLE:
        {
            ...
            sgxbutil::Status st = sgxbutil::Status::OK();
            reset(&st);
            if (should_step_down) {
                _node->step_down(_node->_current_term, true,
                        sgxbutil::Status(ELEADERREMOVED, "This node was removed"));
            }
            return;
        }
    case STAGE_NONE:
...
void NodeImpl::ConfigurationCtx::reset(sgxbutil::Status* st) {
...
void NodeImpl::VoteBallotCtx::start_grant_self_timer(int64_t wait_ms, NodeImpl* node) {
    timespec duetime = sgxbutil::milliseconds_from_now(wait_ms);
...
int64_t NodeImpl::last_leader_active_timestamp(const Configuration& conf) {
    ...
    if (last_rpc_send_timestamps.empty()) {
        return sgxbutil::monotonic_time_ms();
    }
    std::pop_heap(last_rpc_send_timestamps.begin(), last_rpc_send_timestamps.end(), compare);
