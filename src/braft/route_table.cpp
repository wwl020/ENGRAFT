...
friend struct DefaultSingletonTraits<RouteTable>;
    ...
    typedef sgxbutil::FlatMap<GroupId, GroupConf> GroupMap;
    typedef sgxbutil::DoublyBufferedData<GroupMap> DbMap;
...
sgxbutil::Status refresh_leader(const GroupId& group, int timeout_ms) {
    RouteTable* const rtb = RouteTable::GetInstance();
    Configuration conf;
    if (rtb->list_conf(group, &conf) != 0) {
        return sgxbutil::Status(ENOENT, "group %s is not reistered in RouteTable",
                                    group.c_str());
    }
    sgxbutil::Status error;
    for (Configuration::const_iterator
            iter = conf.begin(); iter != conf.end(); ++iter) {
        brpc::Channel channel;
        brpc::ChannelOptions options;
        options.mutable_ssl_options();
        if (channel.Init(iter->addr, &options) != 0) {
            if (error.ok()) {
