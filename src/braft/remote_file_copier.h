...
public:
    ...
    class Session : public sgxbutil::RefCountedThreadSafe<Session> {
    public:
        Session();
        ~Session();
    ...
    friend class Closure;
        ...
        sgxscoped_refptr<SnapshotThrottle> _throttle;   
    ...
    int copy_to_iobuf(const std::string& source,
                      ...
                      const CopyOptions* options);
    sgxscoped_refptr<Session> start_to_copy_to_file(
                      const std::string& source,
                      const std::string& dest_path,
                      const CopyOptions* options);
    sgxscoped_refptr<Session> start_to_copy_to_iobuf(
                      const std::string& source,
                      sgxbutil::IOBuf* dest_buf,
                      const CopyOptions* options);
private:
    int read_piece_of_file(sgxbutil::IOBuf* buf, const std::string& source,
