...
#include "sgxbutil/object_pool.h"                   // butil::get_object
...
void LogManager::append_entries(
            std::vector<LogEntry*> *entries, StableClosure* done) {
    CHECK(done);
    if (_has_error.load(sgxbutil::memory_order_relaxed)) {
        for (size_t i = 0; i < entries->size(); ++i) {
            (*entries)[i]->Release();
        }
        entries->clear();
...
void LogManager::append_to_storage(std::vector<LogEntry*>* to_append, 
    ...
    if (!_has_error.load(sgxbutil::memory_order_relaxed)) {
        //- written_size is used to calculate g_nomralized_append_entries_latency
        ...
        timer.start();
        //- Call SegmentLogStorage::append_entries
        //- nappent is the size that successfully appends
        int nappent = _log_storage->append_entries(*to_append, metric);
        timer.stop();
        // VLOG(79) << "TIME_OF log storage append entries = " << timer.m_elapsed(0) << " ms, nappent = " << nappent;
        ...
        // g_storage_append_entries_latency << timer.u_elapsed(); //- Remove bvar
        if (written_size) {
            // g_nomralized_append_entries_latency << timer.u_elapsed() * 1024 / written_size; //- Remove bvar
        }
    }
...
public:
    ...
    ~AppendBatcher() { flush_force(); }

    void flush_force() {
        if (_size > 0) {
            IOMetric metric;
            _lm->append_to_storage(&_to_append, _last_id, &metric);
            // g_storage_flush_batch_counter << _size; //- Remove bvar
            for (size_t i = 0; i < _size; ++i) {
                _storage[i]->_entries.clear();
                if (_lm->_has_error.load(sgxbutil::memory_order_relaxed)) {
        ...
        _size = 0;
        _buffer_size = 0;
        _last_flush_time = sgxbutil::gettimeofday_ms();
    }

    void flush() {
        if (_size > 0) {
            bool out_of_size = _buffer_size >= (size_t)FLAGS_raft_max_append_buffer_size;
            if (_size == _cap || out_of_size || 
                    (sgxbutil::gettimeofday_ms() - _last_flush_time >= 100)) {
                flush_force();
            }
        }
    }
...
// static pthread_once_t init_ab = PTHREAD_ONCE_INIT;
// void initialize_append_batcher() {

// }
// LogManager::StableClosure* storage[1024];
// AppendBatcher ab(storage, FLAGS_raft_leader_batch, &last_id, log_manager);
int LogManager::disk_thread(void* meta,
    ...
    StableClosure* storage[FLAGS_raft_leader_batch];
    AppendBatcher ab(storage, FLAGS_raft_leader_batch, &last_id, log_manager);
    ...
    for (; iter; ++iter) {
        ...
        done->metric.bthread_queue_time_us = sgxbutil::cpuwide_time_us() - 
...
LogEntry* LogManager::get_entry_from_memory(const int64_t index) {
    ...
    if (!_logs_in_memory.empty()) {
        ...
        // if (entry == NULL) {
        //     VLOG(79) << "Func: " << __FUNCTION__ << " mem entry is NULL, index = "
        //         << index << ", first_index = " << first_index << ", last_index = " << last_index;
        // }
    }
    return entry;
}
...
int64_t LogManager::unsafe_get_term(const int64_t index) {
    ...
    // g_read_term_from_storage << 1; //- Remove bvar
...
LogEntry* LogManager::get_entry(const int64_t index) {
    ...
    lck.unlock();
    // g_read_entry_from_storage << 1; //- Remove bvar

    // VLOG(79) << "Func: " << __FUNCTION__ << " Calling _log_storage->get_entry, index = "
    //     << index << " last_index = " << _last_log_index;
...
void LogManager::set_disk_id(const LogId& disk_id) {
    ...
    lck.unlock();
    // VLOG(79) << "Func: " << __FUNCTION__ << " Calling clear_memory_logs #1";
...
void LogManager::set_applied_id(const LogId& applied_id) {
    ...
    LogId clear_id = std::min(_disk_id, _applied_id);
    lck.unlock();
    // VLOG(79) << "Func: " << __FUNCTION__ << " Calling clear_memory_logs #2";
    return clear_memory_logs(clear_id);
}
...
LogManager::WaitId LogManager::wait(
    ...
    WaitMeta* wm = sgxbutil::get_object<WaitMeta>();
...
void LogManager::wakeup_all_waiter(std::unique_lock<raft_mutex_t>& lck) {
    ...
    size_t nwm = 0;
    for (sgxbutil::FlatMap<int64_t, WaitMeta*>::const_iterator
    ...
    //- bthread_flush is coupled with BTHREAD_NOSIGNAL attr
    // bthread_flush();
}
...
void LogManager::report_error(int error_code, const char* fmt, ...) {
    _has_error.store(true, sgxbutil::memory_order_relaxed);
    va_list ap;
    va_start(ap, fmt);
    Error e;
...
sgxbutil::Status LogManager::check_consistency() {
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    if (_last_snapshot_id == LogId(0, 0)) {
        ...
        return sgxbutil::Status(EIO, "Missing logs in (0, %" PRId64 ")", _first_log_index);
    } else {
        if (_last_snapshot_id.index >= _first_log_index - 1
                && _last_snapshot_id.index <= _last_log_index) {
            return sgxbutil::Status::OK();
        }
        return sgxbutil::Status(EIO, "There's a gap between snapshot={%" PRId64 ", %" PRId64 "}"
                                 " and log=[%" PRId64 ", %" PRId64 "] ",
                            _last_snapshot_id.index, _last_snapshot_id.term,
                            _first_log_index, _last_log_index);
    }
    CHECK(false) << "Can't reach here";
    return sgxbutil::Status(-1, "Impossible condition");
}
