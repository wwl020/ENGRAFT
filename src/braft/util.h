...
#include "sgxbutil/containers/doubly_buffered_data.h"
#include "sgxbutil/third_party/crc32/crc32c.h"
...
inline uint32_t murmurhash32(const void *key, int len) {
    uint32_t hash = 0;
    sgxbutil::MurmurHash3_x86_32(key, len, 0, &hash);
    return hash;
}

inline uint32_t murmurhash32(const sgxbutil::IOBuf& buf) {
    sgxbutil::MurmurHash3_x86_32_Context ctx;
    sgxbutil::MurmurHash3_x86_32_Init(&ctx, 0);
    const size_t block_num = buf.backing_block_num();
    for (size_t i = 0; i < block_num; ++i) {
        sgxbutil::StringPiece sp = buf.backing_block(i);
        if (!sp.empty()) {
            sgxbutil::MurmurHash3_x86_32_Update(&ctx, sp.data(), sp.size());
        }
    }
    uint32_t hash = 0;
    sgxbutil::MurmurHash3_x86_32_Final(&hash, &ctx);
    return hash;
}

inline uint32_t crc32(const void* key, int len) {
    return sgxbutil::crc32c::Value((const char*)key, len);
}

inline uint32_t crc32(const sgxbutil::IOBuf& buf) {
    uint32_t hash = 0;
    const size_t block_num = buf.backing_block_num();
    for (size_t i = 0; i < block_num; ++i) {
        sgxbutil::StringPiece sp = buf.backing_block(i);
        if (!sp.empty()) {
            hash = sgxbutil::crc32c::Extend(hash, sp.data(), sp.size());
        }
    }
    return hash;
}
...
ssize_t file_pread(sgxbutil::IOPortal* portal, int fd, off_t offset, size_t size);

ssize_t file_pwrite(const sgxbutil::IOBuf& data, int fd, off_t offset);

// unsequence file data, reduce the overhead of copy some files have hole.
class FileSegData {
public:
    // for reader
    FileSegData(const sgxbutil::IOBuf& data)
        : _data(data), _seg_header(sgxbutil::IOBuf::INVALID_AREA), _seg_offset(0), _seg_len(0) {}
    // for writer
    FileSegData() : _seg_header(sgxbutil::IOBuf::INVALID_AREA), _seg_offset(0), _seg_len(0) {}
    ~FileSegData() {
        close();
    }

    // writer append
    void append(const sgxbutil::IOBuf& data, uint64_t offset);
    ...
    size_t next(uint64_t* offset, sgxbutil::IOBuf* data);

private:
    void close();

    sgxbutil::IOBuf _data;
    sgxbutil::IOBuf::Area _seg_header;
