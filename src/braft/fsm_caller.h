...
public:
    ...
    Closure* done() const;
    void set_error_and_rollback(size_t ntail, const sgxbutil::Status* st);
...
friend class FSMCaller;
    ...
    LogEntry* _cur_entry;
    sgxbutil::atomic<int64_t>* _applying_index;
    Error _error;
};
...
public:
    ...
    int on_leader_stop(const sgxbutil::Status& status);
    ...
    int64_t last_applied_index() const {
        return _last_applied_index.load(sgxbutil::memory_order_relaxed);
    }
...
friend class IteratorImpl;
    ...
    void do_leader_stop(const sgxbutil::Status& status);
    ...
    sgxbutil::atomic<int64_t> _last_applied_index;
