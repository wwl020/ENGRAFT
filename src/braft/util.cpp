...
ssize_t file_pwrite(const sgxbutil::IOBuf& data, int fd, off_t offset) {
    ...
    sgxbutil::IOBuf piece_data(data);
    off_t orig_offset = offset;
    ssize_t left = size;
    while (left > 0) {
...
void FileSegData::append(const sgxbutil::IOBuf& data, uint64_t offset) {
    ...
    if (0 != _seg_offset && offset == (_seg_offset + _seg_len)) {
        ...
        if (_seg_len > 0) {
            ::sgxbutil::RawPacker(seg_header).pack64(_seg_offset).pack32(_seg_len);
        ...
        CHECK(_seg_header != sgxbutil::IOBuf::INVALID_AREA);
        _data.append(data);
    }
}
...
size_t FileSegData::next(uint64_t* offset, sgxbutil::IOBuf* data) {
    data->clear();
    if (_data.length() == 0) {
        return 0;
    }
    ...
    uint32_t seg_len = 0;
    ::sgxbutil::RawUnpacker(header_buf).unpack64(seg_offset).unpack32(seg_len);
