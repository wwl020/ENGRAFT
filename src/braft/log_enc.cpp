...
int EncryptedSegment::_load_entry(off_t offset, EntryHeader* head, sgxbutil::IOBuf* data,
...
int EncryptedSegment::_extract_log_entry_header(sgxbutil::IOBuf read_iobuf, unsigned char* iv, unsigned char* aad, unsigned char* mac, EntryHeader* head) const {
    size_t buf_offset = 0;
    //- 1. Extract iv
...
int EncryptedSegment::_load_whole_entry(off_t offset, EntryHeader* head, sgxbutil::IOBuf* data,
                         size_t size_hint) const {
...
int EncryptedSegmentLogStorage::append_entries(const std::vector<LogEntry*>& entries, IOMetric* metric) {
    ...
    sgxscoped_refptr<EncryptedSegment> last_segment = NULL;
    int64_t now = 0;
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        sgxscoped_refptr<EncryptedSegment> segment = open_segment();
    ...
    sgxbutil::Timer timer;
    // timer.start();
    //- Increase the store, so save meta firstly
    save_meta(first_log_index(), last_log_index());
    // timer.stop();
...
LogEntry* EncryptedSegmentLogStorage::get_entry(const int64_t index) {
    sgxscoped_refptr<EncryptedSegment> ptr;
...
void EncryptedSegmentLogStorage::pop_segments(
        ...
        std::vector<sgxscoped_refptr<EncryptedSegment> >* popped) {
    popped->clear();
    popped->reserve(32);
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    for (SegmentMap::iterator it = _segments.begin(); it != _segments.end();) {
        sgxscoped_refptr<EncryptedSegment>& segment = it->second;
...
sgxscoped_refptr<EncryptedSegment> EncryptedSegmentLogStorage::open_segment() {
    // sgxscoped_refptr<EncryptedSegment> prev_open_segment;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        if (!_open_segment) {
...
int EncryptedSegmentLogStorage::get_segment(int64_t index, sgxscoped_refptr<EncryptedSegment>* ptr) {
    BAIDU_SCOPED_LOCK(_mutex);
...
void EncryptedSegmentLogStorage::sync() {
    std::vector<sgxscoped_refptr<EncryptedSegment> > segments;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        for (SegmentMap::iterator it = _segments.begin(); it != _segments.end(); ++it) {
...
sgxbutil::Status EncryptedSegmentLogStorage::gc_instance(const std::string& uri) const {
