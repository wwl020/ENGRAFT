...
#include "sgxbutil/files/file.h"                        // sgxbutil::File
#include "sgxbutil/files/dir_reader_posix.h"            // butil::DirReaderPosix
#include "sgxbutil/memory/ref_counted.h"                // sgxbutil::RefCountedThreadSafe
#include "sgxbutil/memory/singleton.h"                  // Singleton
...
public:
    ...
    virtual ssize_t write(const sgxbutil::IOBuf& data, off_t offset) = 0;
    ...
    virtual ssize_t read(sgxbutil::IOPortal* portal, off_t offset, size_t size) = 0;
...
class FileSystemAdaptor : public sgxbutil::RefCountedThreadSafe<FileSystemAdaptor> {
public:
    FileSystemAdaptor() {}
...
private:
    sgxbutil::DirReaderPosix _dir_reader;
};
...
public:
    ...
    virtual ssize_t write(const sgxbutil::IOBuf& data, off_t offset);
    virtual ssize_t read(sgxbutil::IOPortal* portal, off_t offset, size_t size);
...
// Convert sgxbutil::File::Error to os error
...
bool create_sub_directory(const std::string& parent_path,
                          ...
                          sgxbutil::File::Error* error = NULL);
