...
#include "sgxbutil/iobuf.h"                        // sgxbutil::IOBuf, IOPortal
#include "sgxbutil/macros.h"                       // DISALLOW_COPY_AND_ASSIGN
#include "sgxbutil/endpoint.h"                     // sgxbutil::EndPoint
#include "sgxbutil/resource_pool.h"                // sgxbutil::ResourceId
...
public:
    ...
    virtual ssize_t CutMessageIntoFileDescriptor(int, sgxbutil::IOBuf**, size_t) = 0;
    virtual ssize_t CutMessageIntoSSLChannel(SSL*, sgxbutil::IOBuf**, size_t) = 0;
};
...
friend class HostEventDispatcher;
...
public:
    ...
    int Write(sgxbutil::IOBuf *msg, const WriteOptions* options = NULL);
    ...
    // The file descriptor
    int fd() const { return _fd.load(sgxbutil::memory_order_relaxed); }
    ...
    { return _parsing_context.load(sgxbutil::memory_order_consume); }
    ...
    { return _recycle_flag.load(sgxbutil::memory_order_relaxed); }
    ...
    // This function can only be called after a successful
    ...
    int64_t last_active_time_us() const {
        return std::max(
            _last_readtime_us.load(sgxbutil::memory_order_relaxed),
            _last_writetime_us.load(sgxbutil::memory_order_relaxed));
    }
...
private:
    ...
    sgxbutil::atomic<uint64_t> _versioned_ref;
    ...
    sgxbutil::atomic<SharedPart*> _shared_part;
    ...
    sgxbutil::atomic<int> _nevent;

    
    // [ Set in ResetFileDescriptor ] 
    sgxbutil::atomic<int> _fd;  // -1 when not connected.
    ...
    sgxbutil::IOPortal _read_buf;

    // Set with cpuwide_time_us() at last read operation
    sgxbutil::atomic<int64_t> _last_readtime_us;

    // Saved context for parsing, reset before trying other protocols.
    sgxbutil::atomic<Destroyable*> _parsing_context;
    ...
    //- This variable is always 0 in sgx-braft.
    ...
    sgxbutil::atomic<uint32_t> _ninprocess;
    ...
    sgxbutil::atomic<uint64_t> _auth_flag_error;
    ...
    sgxbutil::atomic<bool> _controller_released_socket;
    ...
    sgxbutil::atomic<SocketId> _agent_socket_id;

    sgxbutil::Mutex _pipeline_mutex;
    ...
    sgxbutil::atomic<int64_t> _last_writetime_us;
    // Queued but written
    sgxbutil::atomic<int64_t> _unwritten_bytes;
    
    // Butex to wait for EPOLLOUT event
    sgxbutil::atomic<int>* _epollout_butex;

    // Storing data that are not flushed into `fd' yet.
    sgxbutil::atomic<WriteRequest*> _write_head;

    sgxbutil::Mutex _stream_mutex;
};
