...
uint32_t MD5Hash32V(const sgxbutil::StringPiece* keys, size_t num_keys) {
    MD5_CTX ctx;
    MD5_Init(&ctx);
...
uint32_t MurmurHash32(const void* key, size_t len) {
    uint32_t hash;
    sgxbutil::MurmurHash3_x86_32(key, (int)len, 0, &hash);
    return hash;
}

uint32_t MurmurHash32V(const sgxbutil::StringPiece* keys, size_t num_keys) {
    sgxbutil::MurmurHash3_x86_32_Context ctx;
    sgxbutil::MurmurHash3_x86_32_Init(&ctx, 0);
    for (size_t i = 0; i < num_keys; ++i) {
        sgxbutil::MurmurHash3_x86_32_Update(&ctx, keys[i].data(), keys[i].size());
    }
    uint32_t hash;
