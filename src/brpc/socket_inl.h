...
MakeSocketId(uint32_t version, sgxbutil::ResourceId<Socket> slot) {
    return SocketId((((uint64_t)version) << 32) | slot.value);
}

BUTIL_FORCE_INLINE sgxbutil::ResourceId<Socket> SlotOfSocketId(SocketId sid) {
    sgxbutil::ResourceId<Socket> id = { (sid & 0xFFFFFFFFul) };
    return id;
}
...
inline int Socket::Dereference() {
    ...
    const uint64_t vref = _versioned_ref.fetch_sub(
        1, sgxbutil::memory_order_release);
...
inline int Socket::Address(SocketId id, SocketUniquePtr* ptr) {
    ...
    if (__builtin_expect(m != NULL, 1)) {
        ...
        const uint64_t vref1 = m->_versioned_ref.fetch_add(
            1, sgxbutil::memory_order_acquire);
...
inline void Socket::ReAddress(SocketUniquePtr* ptr) {
    _versioned_ref.fetch_add(1, sgxbutil::memory_order_acquire);
    ptr->reset(this);
}
...
inline bool Socket::Failed() const {
    return VersionOfVRef(_versioned_ref.load(sgxbutil::memory_order_relaxed))
        != VersionOfSocketId(_this_id);
}

//- It seems that the value of progress is always 1, 
//- which is set in socket.h (Socket::PROGRESS_INIT)
inline bool Socket::MoreReadEvents(int* progress) {
    // Fail to CAS means that new events arrived.
    //- contained.compare_exchange_strong (T& expected, T val, memory_order success, memory_order failure) noexcept;
    ...
    //- Return true if expected compares equal to the contained value, and false otherwise.
    /** 
    We already know progress = 1, if:
        _nevent = 0, change progress to 0, and func compare_exchange_strong() return true,
        and !compare_exchange_strong() will return false, which means no more events.

        _nevent > 0, change _nevent to 0, and func compare_exchange_strong() return false;
        and !compare_exchange_strong() will return true, which means there are more events.
     **/

    //- What's more, the value of _nevent is only changed in Socket::StartInputEvent func. 
    return !_nevent.compare_exchange_strong(
        *progress, 0, sgxbutil::memory_order_release,
            sgxbutil::memory_order_acquire);
}

inline void Socket::SetLogOff() {
    if (!_logoff_flag.exchange(true, sgxbutil::memory_order_relaxed)) {
        if (fd() < 0) {
...
inline bool Socket::IsAvailable() const {
    return !_logoff_flag.load(sgxbutil::memory_order_relaxed);
}
...
inline void Socket::PostponeEOF() {
    if (CreatedByConnect()) { // not needed at server-side
        _ninprocess.fetch_add(1, sgxbutil::memory_order_relaxed);
    }
}
...
inline void Socket::CheckEOFInternal() {
    uint32_t nref = _ninprocess.fetch_sub(1, sgxbutil::memory_order_release);
    if ((nref & ~EOF_FLAG) == 1) {
...
inline void Socket::SetEOF() {
    uint32_t nref = _ninprocess.fetch_or(EOF_FLAG, sgxbutil::memory_order_relaxed);
    if ((nref & EOF_FLAG) == 0) {
...
inline void Socket::reset_parsing_context(Destroyable* new_context) {
    Destroyable* old_ctx = _parsing_context.exchange(
        new_context, sgxbutil::memory_order_acq_rel);
    if (old_ctx) {
        old_ctx->Destroy();
    }
}
...
inline Destroyable* Socket::release_parsing_context() {
    return _parsing_context.exchange(NULL, sgxbutil::memory_order_acquire);
}
...
bool Socket::initialize_parsing_context(T** ctx) {
    ...
    if (_parsing_context.compare_exchange_strong(
            expected, *ctx, sgxbutil::memory_order_acq_rel,
            sgxbutil::memory_order_acquire)) {
        return true;
    } else {
        (*ctx)->Destroy();
...
inline Socket::SharedPart* Socket::GetSharedPart() const {
    return _shared_part.load(sgxbutil::memory_order_consume);
}
