...
#include <brpc/callback.h> //- NewCallback
...
#include "sgxbutil/time.h"                           // cpuwide_time_us
#include "sgxbutil/object_pool.h"                    // get_object
#include "sgxbutil/logging.h"                        // CHECK
...
private:
    ...
    sgxbutil::atomic<int> _numfree; // #free sockets in all sub pools.
    sgxbutil::atomic<int> _numinflight; // #inflight sockets in all sub pools.
};
...
public:
    ...
    sgxbutil::atomic<size_t> in_size;
    sgxbutil::atomic<size_t> in_num_messages;
    sgxbutil::atomic<size_t> out_size;
    sgxbutil::atomic<size_t> out_num_messages;

    // For computing stats.
    ExtendedSocketStat* extended_stat;

    sgxbutil::atomic<uint64_t> recent_error_count;
...
Socket::SharedPart::~SharedPart() {
    ...
    extended_stat = NULL;
    delete socket_pool.exchange(NULL, sgxbutil::memory_order_relaxed);
}
...
void Socket::SharedPart::UpdateStatsEverySecond(int64_t now_ms) {
    ...
    const size_t in_sz = in_size.load(sgxbutil::memory_order_relaxed);
    const size_t in_nmsg = in_num_messages.load(sgxbutil::memory_order_relaxed);
    const size_t out_sz = out_size.load(sgxbutil::memory_order_relaxed);
    const size_t out_nmsg = out_num_messages.load(sgxbutil::memory_order_relaxed);
...
void Socket::WriteRequest::Setup(Socket* s) {
    ...
    if (msg) {
        clear_user_message();
        if (msg != DUMMY_USER_MESSAGE) {
            sgxbutil::Status st = msg->AppendAndDestroySelf(&data, s);
            if (!st.ok()) {
                // Abandon the request.
                data.clear();
        ...
        const int64_t before_write =
            s->_unwritten_bytes.fetch_add(data.size(), sgxbutil::memory_order_relaxed);
    ...
    , _write_head(NULL)
{
    ...
    _epollout_butex = bthread::butex_create_checked<sgxbutil::atomic<int> >();
}
...
int Socket::ResetFileDescriptor(int fd) {
    ...
    _fd.store(fd, sgxbutil::memory_order_release);
    _reset_fd_real_us = sgxbutil::gettimeofday_us();
    if (!ValidFileDescriptor(fd)) {
        return 0;
    }
    // OK to fail, non-socket fd does not support this.
    if (sgxbutil::get_local_side(fd, &_local_side) != 0) {
    ...
    if (_on_edge_triggered_events) {
        if (GetGlobalEventDispatcher(fd).AddConsumer(id(), fd) != 0) {
            ...
            _fd.store(-1, sgxbutil::memory_order_release);
            return -1;
        }
    }
    return 0;
...
int Socket::Create(const SocketOptions& options, SocketId* id) {
    sgxbutil::ResourceId<Socket> slot;
    Socket* const m = sgxbutil::get_resource(&slot, Forbidden());
    if (m == NULL) {
        LOG(FATAL) << "Fail to get_resource<Socket>";
        return -1;
    }
    // g_vars->nsocket << 1;
    CHECK(NULL == m->_shared_part.load(sgxbutil::memory_order_relaxed));
    m->_nevent.store(0, sgxbutil::memory_order_relaxed);
    m->_tos = 0;
    ...
    m->_this_id = MakeSocketId(
            VersionOfVRef(m->_versioned_ref.fetch_add(
                    1, sgxbutil::memory_order_release)), slot);
    m->_preferred_index = -1;
    m->_hc_count = 0;
    CHECK(m->_read_buf.empty());
    const int64_t cpuwide_now = sgxbutil::cpuwide_time_us();
    m->_last_readtime_us.store(cpuwide_now, sgxbutil::memory_order_relaxed);
    m->reset_parsing_context(options.initial_parsing_context);
    m->_correlation_id = 0;
    m->_ninprocess.store(1, sgxbutil::memory_order_relaxed);
    m->_auth_flag_error.store(0, sgxbutil::memory_order_relaxed);
    ...
    m->_controller_released_socket.store(false, sgxbutil::memory_order_relaxed);
    m->_overcrowded = false;
    // May be non-zero for RTMP connections.
    m->_fail_me_at_server_stop = false;
    m->_logoff_flag.store(false, sgxbutil::memory_order_relaxed);
    m->_recycle_flag.store(false, sgxbutil::memory_order_relaxed);
    m->_error_code = 0;
    m->_error_text.clear();
    m->_agent_socket_id.store(INVALID_SOCKET_ID, sgxbutil::memory_order_relaxed);
    ...
    m->_last_writetime_us.store(cpuwide_now, sgxbutil::memory_order_relaxed);
    m->_unwritten_bytes.store(0, sgxbutil::memory_order_relaxed);
    CHECK(NULL == m->_write_head.load(sgxbutil::memory_order_relaxed));
...
int Socket::WaitAndReset(int32_t expected_nref) {
    ...
    while (1) {
        ...
        vref = _versioned_ref.load(sgxbutil::memory_order_acquire);
    ...
    const int prev_fd = _fd.exchange(-1, sgxbutil::memory_order_relaxed);
    ...
    _ssl_state = SSL_UNKNOWN;
    _nevent.store(0, sgxbutil::memory_order_relaxed);
    ...
    _read_buf.clear();
    _ninprocess.store(1, sgxbutil::memory_order_relaxed);
    _auth_flag_error.store(0, sgxbutil::memory_order_relaxed);
    ...
    const int64_t cpuwide_now = sgxbutil::cpuwide_time_us();
    _last_readtime_us.store(cpuwide_now, sgxbutil::memory_order_relaxed);
    _last_writetime_us.store(cpuwide_now, sgxbutil::memory_order_relaxed);
    _logoff_flag.store(false, sgxbutil::memory_order_relaxed);
    {
    ...
    if (sp) {
        sp->recent_error_count.store(0, sgxbutil::memory_order_relaxed);
    }
    return 0;
}
...
void Socket::AddRecentError() {
    SharedPart* sp = GetSharedPart();
    if (sp) {
        sp->recent_error_count.fetch_add(1, sgxbutil::memory_order_relaxed);
    }
}
...
int64_t Socket::recent_error_count() const {
    SharedPart* sp = GetSharedPart();
    if (sp) {
        return sp->recent_error_count.load(sgxbutil::memory_order_relaxed);
    }
    return 0;
}

//- TODO: 该函数和服务熔断有关，后期去掉
...
int Socket::SetFailed(int error_code, const char* error_fmt, ...) {
    ...
    const uint32_t id_ver = VersionOfSocketId(_this_id);
    uint64_t vref = _versioned_ref.load(sgxbutil::memory_order_relaxed);
    for (;;) {  // need iteration to retry compare_exchange_strong
        ...
        if (_versioned_ref.compare_exchange_strong(
            ...
            if (error_fmt != NULL) {
                ...
                sgxbutil::string_vprintf(&error_text, error_fmt, ap);
                va_end(ap);
            }
            pthread_mutex_lock(&_id_wait_list_mutex);
            ...
            _epollout_butex->fetch_add(1, sgxbutil::memory_order_relaxed);
...
int Socket::ReleaseReferenceIfIdle(int idle_seconds) {
    ...
    if (sgxbutil::cpuwide_time_us() - last_active_us <= idle_seconds * 1000000L) {
        return 0;
    }
...
int Socket::Status(SocketId id, int32_t* nref) {
    const sgxbutil::ResourceId<Socket> slot = SlotOfSocketId(id);
    Socket* const m = address_resource(slot);
    if (m != NULL) {
        const uint64_t vref = m->_versioned_ref.load(sgxbutil::memory_order_relaxed);
...
void Socket::OnRecycle() {
    ...
    SharedPart* sp = _shared_part.exchange(NULL, sgxbutil::memory_order_acquire);
    if (sp) {
    ...
    const SocketId asid = _agent_socket_id.load(sgxbutil::memory_order_relaxed);
...
void* Socket::ProcessEvent(void* arg) {
    LOG(INFO) << "Func: " << __FUNCTION__ << " bthread-" << bthread_self() << " Processing input event";
    VLOG(80) << "Func: " << __FUNCTION__ << " bthread-" << bthread_self() << " Processing input event";
    // the enclosed Socket is valid and free to access inside this function.
    SocketUniquePtr s(static_cast<Socket*>(arg));
    //- _on_edge_triggered_events may be Acceptor::OnNewConnections() or 
...
bool Socket::IsWriteComplete(Socket::WriteRequest* old_head,
    ...
    if (_write_head.compare_exchange_strong(
            new_head, desired, sgxbutil::memory_order_acquire)) {
...
int Socket::WaitEpollOut(int fd, bool pollin, const timespec* abstime) {
    ...
    const int expected_val = _epollout_butex->load(sgxbutil::memory_order_relaxed);
    ...
    sgxbutil::ignore_result(edisp.RemoveEpollOut(id(), fd, pollin));
    errno = saved_errno;
    // Could be writable or spurious wakeup (by former epollout)
    return rc;
}

//- The on_connect function pointer may be NULL or Socket::KeepWriteIfConnected
//- However, in sgx-braft, on_connect is always Socket::KeepWriteIfConnected.
int Socket::Connect(const timespec* abstime,
    ...
    sgxbutil::fd_guard sockfd(socket(AF_INET, SOCK_STREAM, 0));
    if (sockfd < 0) {
        PLOG(ERROR) << "Fail to create socket";
        return -1;
    }
    CHECK_EQ(0, sgxbutil::make_close_on_exec(sockfd));
    // We need to do async connect (to manage the timeout by ourselves).
    CHECK_EQ(0, sgxbutil::make_non_blocking(sockfd));
    ...
    //- Here, non-blocking connect is used, and EINPROGRESS means connecting (we don't know whether the socket is connected successfully now).
...
int Socket::ConnectIfNot(const timespec* abstime, WriteRequest* req) {
    if (_fd.load(sgxbutil::memory_order_consume) >= 0) {
       return 0;
    }
    ...
    ReAddress(&s);
    req->socket = s.get();
    LOG(INFO) << "Func: " << __FUNCTION__ << " bthread-" << bthread_self() << " Not connected yet, remote party = " << req->socket->remote_side();
    if (_conn) {
...
int Socket::HandleEpollOut(SocketId id) {
    ...
    s->_epollout_butex->fetch_add(1, sgxbutil::memory_order_relaxed);
...
void Socket::AfterAppConnected(int err, void* data) {
    ...
    if (err == 0) {
        ...
        if (sp) {
            sp->num_continuous_connect_timeouts.store(0, sgxbutil::memory_order_relaxed);
        }
        ...
        if (err == ETIMEDOUT) {
            ...
            if (sp->num_continuous_connect_timeouts.fetch_add(
                    1, sgxbutil::memory_order_relaxed) + 1 >=
                ...
                sp->num_continuous_connect_timeouts.store(
                    0, sgxbutil::memory_order_relaxed);
                err = ENETUNREACH;
            }
        }
...
int Socket::Write(sgxbutil::IOBuf* data, const WriteOptions* options_in) {
    WriteOptions opt;
    if (options_in) {
        opt = *options_in;
    }
    ...
    WriteRequest* req = sgxbutil::get_object<WriteRequest>();
    if (!req) {
    ...
    LOG(INFO) << "Func: " << __FUNCTION__ << " bthread-" << bthread_self() << " opt.pipelined_count = " << opt.pipelined_count << " opt.with_auth = " << opt.with_auth;
    return StartWrite(req, opt);
}

//- This func seems not be called in braft.
...
int Socket::StartWrite(WriteRequest* req, const WriteOptions& opt) {
    ...
    WriteRequest* const prev_head =
        _write_head.exchange(req, sgxbutil::memory_order_release);
    if (prev_head != NULL) {
    ...
    //- If pipelined_count is 0 (normal case in braft), this can be omitted.
    req->Setup(this);
    ...
    if (_conn) {
        sgxbutil::IOBuf* data_arr[1] = { &req->data };
...
void* Socket::KeepWrite(void* void_arg) {    
    ...
    SocketUniquePtr s(req->socket);
    LOG(INFO) << "Func: " << __FUNCTION__ << " bthread-" << bthread_self() << " SocketId = " << s->_this_id;
    ...
    do {
        ...
        if (nw <= 0) {
            ...
            const timespec duetime =
                sgxbutil::milliseconds_from_now(WAIT_EPOLLOUT_TIMEOUT_MS);
...
ssize_t Socket::DoWrite(WriteRequest* req) {
    // Group sgxbutil::IOBuf in the list into a batch array.
    sgxbutil::IOBuf* data_list[DATA_LIST_MAX];
    size_t ndata = 0;
    ...
    if (ssl_state() == SSL_OFF) {
        ...
        if (_conn) {
            ...
            ssize_t nw = sgxbutil::IOBuf::cut_multiple_into_file_descriptor(
                fd(), data_list, ndata);
            return nw;
        }
    }
    ...
    int ssl_error = 0;
    ssize_t nw = sgxbutil::IOBuf::cut_multiple_into_SSL_channel(
...
int Socket::FightAuthentication(int* auth_error) {
    ...
    uint64_t flag_error = _auth_flag_error.load(sgxbutil::memory_order_relaxed);
    ...
    if (0 == bthread_id_trylock(_auth_id, NULL)) {
        ...
        flag_error = _auth_flag_error.load(sgxbutil::memory_order_relaxed);
        *auth_error = (int32_t)(flag_error & 0xFFFFFFFFul);
        return EINVAL;
    }
}
...
int Socket::StartInputEvent(SocketId id, uint32_t events,
    ...
    if (s->_nevent.fetch_add(1, sgxbutil::memory_order_acq_rel) == 0) {
...
std::ostream& operator<<(std::ostream& os, const ObjectPtr<T>& obj) {
    if (obj._obj != NULL) {
        // os << '(' << sgxbutil::class_name_str(*obj._obj) << "*)";
    }
    return os << obj._obj;
}
...
void Socket::DebugSocket(std::ostream& os, SocketId id) {
    ...
    const uint64_t vref = ptr->_versioned_ref.load(sgxbutil::memory_order_relaxed);
    size_t npipelined = 0;
    size_t idsizes[4];
    size_t nidsize = 0;
    {
    ...
    if (sp) {
        os << "\nshared_part={\n  ref_count=" << sp->ref_count()
           << "\n  socket_pool=";
        SocketPool* pool = sp->socket_pool.load(sgxbutil::memory_order_consume);
        if (pool) {
            os << '[';
            ...
            os << "]\n  numfree="
               << pool->_numfree.load(sgxbutil::memory_order_relaxed)
               << "\n  numinflight="
               << pool->_numinflight.load(sgxbutil::memory_order_relaxed);
        } else {
            os << "null";
        }
        os << "\n  creator_socket=" << sp->creator_socket_id
           << "\n  in_size=" << sp->in_size.load(sgxbutil::memory_order_relaxed)
           << "\n  in_num_messages=" << sp->in_num_messages.load(sgxbutil::memory_order_relaxed)
           << "\n  out_size=" << sp->out_size.load(sgxbutil::memory_order_relaxed)
           << "\n  out_num_messages=" << sp->out_num_messages.load(sgxbutil::memory_order_relaxed)
           << "\n}";
    }
    const int fd = ptr->_fd.load(sgxbutil::memory_order_relaxed);
    os << "\nnref=" << NRefOfVRef(vref) - 1
        //                                ^
        // minus the ref of current callsite(calling PrintSocket)
       << "\nnevent=" << ptr->_nevent.load(sgxbutil::memory_order_relaxed)
       << "\nfd=" << fd
       << "\ntos=" << ptr->_tos
       << "\nreset_fd_to_now=" << sgxbutil::gettimeofday_us() - ptr->_reset_fd_real_us << "us"
    ...
    os << "\nhc_count=" << ptr->_hc_count
       << "\navg_input_msg_size=" << ptr->_avg_msg_size
        // NOTE: We're assuming that sgxbutil::IOBuf.size() is thread-safe, it is now
    ...
    if (parsing_context_desc) {
        // os << "\nparsing_context=" << sgxbutil::class_name_str(*parsing_context) << '{';
        DescribeOptions opt;
        opt.verbose = true;
    ...
    os << "\npipeline_q=" << npipelined
       << "\n_app_connect=" << ptr->_app_connect
       << "\n_conn=" << ptr->_conn
       << "\nninprocess=" << ptr->_ninprocess.load(sgxbutil::memory_order_relaxed)
       << "\nauth_flag_error=" << ptr->_auth_flag_error.load(sgxbutil::memory_order_relaxed)
       << "\nauth_id=" << ptr->_auth_id.value
       << "\nauth_context=" << ptr->_auth_context
       << "\nlogoff_flag=" << ptr->_logoff_flag.load(sgxbutil::memory_order_relaxed)
       << "\nrecycle_flag=" << ptr->_recycle_flag.load(sgxbutil::memory_order_relaxed)
       << "\nagent_socket_id=";
    const SocketId asid = ptr->_agent_socket_id.load(sgxbutil::memory_order_relaxed);
    if (asid != INVALID_SOCKET_ID) {
        os << asid;
    } else {
        os << "(none)";
    ...
    os << "\ncid=" << ptr->_correlation_id
       << "\nwrite_head=" << ptr->_write_head.load(sgxbutil::memory_order_relaxed)
...
inline int SocketPool::GetSocket(SocketUniquePtr* ptr) {
    ...
    if (connection_pool_size > 0) {
        for (;;) {
            ...
            _numfree.fetch_sub(1, sgxbutil::memory_order_relaxed);
            // Not address inside the lock since at most time the pooled socket
            // is likely to be valid.
            if (Socket::Address(sid, ptr) == 0) {
                _numinflight.fetch_add(1, sgxbutil::memory_order_relaxed);
                return 0;
            }
        }
    }
...
inline void SocketPool::ReturnSocket(Socket* sock) {
    ...
    if (_numfree.fetch_add(1, sgxbutil::memory_order_relaxed) <
    ...
    _numinflight.fetch_sub(1, sgxbutil::memory_order_relaxed);
}
...
Socket::SharedPart* Socket::GetOrNewSharedPartSlower() {
    ...
    if (shared_part == NULL) {
        ...
        if (!_shared_part.compare_exchange_strong(
                expected, shared_part, sgxbutil::memory_order_acq_rel)) {
...
void Socket::ShareStats(Socket* main_socket) {
    ...
    SharedPart* my_sp =
        _shared_part.exchange(main_sp, sgxbutil::memory_order_acq_rel);
    if (my_sp) {
...
int Socket::GetPooledSocket(SocketUniquePtr* pooled_socket) {
    ...
    SocketPool* socket_pool = main_sp->socket_pool.load(sgxbutil::memory_order_consume);
    if (socket_pool == NULL) {
        SocketOptions opt;
        ...
        if (!main_sp->socket_pool.compare_exchange_strong(
                expected, socket_pool, sgxbutil::memory_order_acq_rel)) {
            delete socket_pool;
            CHECK(expected);
...
int Socket::ReturnToPool() {
    ...
    _controller_released_socket.store(false, sgxbutil::memory_order_relaxed);
...
bool Socket::HasSocketPool() const {
    SharedPart* sp = GetSharedPart();
    if (sp != NULL) {
        return sp->socket_pool.load(sgxbutil::memory_order_consume) != NULL;
    }
    return false;
}
...
bool Socket::GetPooledSocketStats(int* numfree, int* numinflight) {
    ...
    *numfree = pool->_numfree.load(sgxbutil::memory_order_relaxed);
    *numinflight = pool->_numinflight.load(sgxbutil::memory_order_relaxed);
    return true;
}
...
int Socket::GetAgentSocket(SocketUniquePtr* out, bool (*checkfn)(Socket*)) {
    SocketId id = _agent_socket_id.load(sgxbutil::memory_order_relaxed);
    ...
    do {
        ...
        if (_agent_socket_id.compare_exchange_strong(
                id, tmp_sock->id(), sgxbutil::memory_order_acq_rel)) {
            out->swap(tmp_sock);
            return 0;
        }
        tmp_sock->ReleaseAdditionalReference();
...
void Socket::AddInputBytes(size_t bytes) {
    GetOrNewSharedPart()->in_size.fetch_add(bytes, sgxbutil::memory_order_relaxed);
}
void Socket::AddInputMessages(size_t count) {
    GetOrNewSharedPart()->in_num_messages.fetch_add(count, sgxbutil::memory_order_relaxed);
}
void Socket::CancelUnwrittenBytes(size_t bytes) {
    const int64_t before_minus =
        _unwritten_bytes.fetch_sub(bytes, sgxbutil::memory_order_relaxed);
    if (before_minus < (int64_t)bytes + FLAGS_socket_max_unwritten_bytes) {
        _overcrowded = false;
    }
}
void Socket::AddOutputBytes(size_t bytes) {
    GetOrNewSharedPart()->out_size.fetch_add(bytes, sgxbutil::memory_order_relaxed);
    _last_writetime_us.store(sgxbutil::cpuwide_time_us(),
                             sgxbutil::memory_order_relaxed);
    CancelUnwrittenBytes(bytes);
}
void Socket::AddOutputMessages(size_t count) {
    GetOrNewSharedPart()->out_num_messages.fetch_add(count, sgxbutil::memory_order_relaxed);
}
...
std::string Socket::description() const {
    ...
    std::string result;
    result.reserve(64);
    sgxbutil::string_appendf(&result, "Socket{id=%" PRIu64, id());
    const int saved_fd = fd();
    if (saved_fd >= 0) {
        sgxbutil::string_appendf(&result, " fd=%d", saved_fd);
    }
    sgxbutil::string_appendf(&result, " addr=%s",
                          sgxbutil::endpoint2str(remote_side()).c_str());
    const int local_port = local_side().port;
    if (local_port > 0) {
        sgxbutil::string_appendf(&result, ":%d", local_port);
    }
    sgxbutil::string_appendf(&result, "} (0x%p)", this);
    return result;
}
