...
//- ThreadRestrictions code is removed since we don't need this kinds of 
//- thread check in sgx-braft
namespace sgxbutil {
...
bool VerifySpecificPathControlledByUser(const FilePath& path,
  ...
  //- TODO: OE don't support lstat, so change temporarily, and this is fine.
...
int CreateAndOpenFdForTemporaryFile(FilePath directory, FilePath* path) {
  ...
  *path = directory.Append(sgxbutil::TempFileName());
  ...
  while (success && !current.empty()) {
    ...
    if (S_ISDIR(from_stat.st_mode)) {
      if (mkdir(target_path.value().c_str(), from_stat.st_mode & 01777) != 0 &&
          errno != EEXIST) {
        // DLOG(ERROR) << "CopyDirectory() couldn't create directory: " << target_path.value() << " errno = " << errno;
        success = false;
      }
    } else if (S_ISREG(from_stat.st_mode)) {
      if (!CopyFile(current, target_path)) {
        // DLOG(ERROR) << "CopyDirectory() couldn't create file: " << target_path.value();
        success = false;
      }
    } else {
