...
#ifndef SGX_BUTIL_IOBUF_H
#define SGX_BUTIL_IOBUF_H
...
#include "sgxbutil/third_party/snappy/snappy-sinksource.h"
#include "sgxbutil/zero_copy_stream_as_streambuf.h"
#include "sgxbutil/macros.h"
#include "sgxbutil/reader_writer.h"
#include "sgxbutil/binary_printer.h"
...
public:
    ...
    // Explicitly declare this overload as error to avoid copy_to(sgxbutil::IOBuf*)
    ...
    bool equals(const sgxbutil::StringPiece&) const;
...
inline bool operator==(const sgxbutil::IOBuf& b, const sgxbutil::StringPiece& s)
{ return b.equals(s); }
inline bool operator==(const sgxbutil::StringPiece& s, const sgxbutil::IOBuf& b)
{ return b.equals(s); }
inline bool operator!=(const sgxbutil::IOBuf& b, const sgxbutil::StringPiece& s)
{ return !b.equals(s); }
inline bool operator!=(const sgxbutil::StringPiece& s, const sgxbutil::IOBuf& b)
{ return !b.equals(s); }
inline bool operator==(const sgxbutil::IOBuf& b1, const sgxbutil::IOBuf& b2)
{ return b1.equals(b2); }
inline bool operator!=(const sgxbutil::IOBuf& b1, const sgxbutil::IOBuf& b2)
{ return !b1.equals(b2); }
...
class IOBufCutter {
public:
    explicit IOBufCutter(sgxbutil::IOBuf* buf);
    ~IOBufCutter();

    // Cut off n bytes and APPEND to `out'
    // Returns bytes cut.
    size_t cutn(sgxbutil::IOBuf* out, size_t n);
    size_t cutn(std::string* out, size_t n);
    size_t cutn(void* out, size_t n);
...
class IOBufAsSnappySource : public sgxbutil::snappy::Source {
public:
    explicit IOBufAsSnappySource(const sgxbutil::IOBuf& buf)
...
private:
    const sgxbutil::IOBuf* _buf;
    sgxbutil::IOBufAsZeroCopyInputStream _stream;
};

// Wrap IOBuf into output of snappy compression.
class IOBufAsSnappySink : public sgxbutil::snappy::Sink {
public:
    explicit IOBufAsSnappySink(sgxbutil::IOBuf& buf);
...
private:
    ...
    int _cur_len;
    sgxbutil::IOBuf* _buf;
    sgxbutil::IOBufAsZeroCopyOutputStream _buf_stream;
};
...
public:
    ...
    int append(const sgxbutil::StringPiece& str);
...
public:
    explicit IOBufBytesIterator(const sgxbutil::IOBuf& buf);
    ...
    size_t append_and_forward(sgxbutil::IOBuf* buf, size_t n);
...
namespace std {
template <>
inline void swap(sgxbutil::IOBuf& a, sgxbutil::IOBuf& b) {
    return a.swap(b);
}
} // namespace std

#include "sgxbutil/iobuf_inl.h"

#endif  // SGX_BUTIL_IOBUF_H
