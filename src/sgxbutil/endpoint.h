...
#ifndef SGX_BUTIL_ENDPOINT_H
#define SGX_BUTIL_ENDPOINT_H

#include <netinet/in.h>                          // in_addr
#include <iostream>                              // std::ostream
#include "sgxbutil/containers/hash_tables.h"         // hashing functions
...
inline bool operator<(sgxbutil::ip_t lhs, sgxbutil::ip_t rhs) {
    return sgxbutil::ip2int(lhs) < sgxbutil::ip2int(rhs);
}
inline bool operator>(sgxbutil::ip_t lhs, sgxbutil::ip_t rhs) {
    return rhs < lhs;
}
...
inline std::ostream& operator<<(std::ostream& os, const sgxbutil::IPStr& ip_str) {
    return os << ip_str.c_str();
}
inline std::ostream& operator<<(std::ostream& os, sgxbutil::ip_t ip) {
    return os << sgxbutil::ip2str(ip);
}
...
template <>
struct hash<sgxbutil::EndPoint> {
    std::size_t operator()(const sgxbutil::EndPoint& ep) const {
        return sgxbutil::HashPair(sgxbutil::ip2int(ep.ip), ep.port);
    }
};

}

#endif  // SGX_BUTIL_ENDPOINT_H
