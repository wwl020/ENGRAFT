...
//- TODO: OE only supports writev/readv (sys/uio.h), so we use
//- writev/readv (use user_preadv/writev namely) to substitute pwritev/preadv
...
//- TODO: We don't use butil/logging in sgx-braft, so we comment relative code(CHECK, LOG)
#include "sgxbutil/logging.h"                  // CHECK, LOG
#include "sgxbutil/fd_guard.h"                 // sgxbutil::fd_guard
...
typedef ssize_t (*iov_function_no_offset)(int fd, const struct iovec *vector, int count);
...
static ssize_t user_pwritev(int fd, const struct iovec* vector,
                            int count, off_t offset) {
    // VLOG(0) << "Func: " << __FUNCTION__ << " fd = " << fd << ", count = " << count << 
        // ", offset = " << offset;
    ssize_t total_write = 0;
    for (int i = 0; i < count; ++i) {
        // VLOG(0) << "Func: " << __FUNCTION__ << " Calling pwrite";
...
inline iov_function_no_offset get_writev_func() {
#ifndef RUN_OUTSIDE_SGX    
    return disk_file_writev;
#else
    return ::writev;
#endif        
}
...
sgxbutil::static_atomic<size_t> g_nblock = BUTIL_STATIC_ATOMIC_INIT(0);
sgxbutil::static_atomic<size_t> g_blockmem = BUTIL_STATIC_ATOMIC_INIT(0);
sgxbutil::static_atomic<size_t> g_newbigview = BUTIL_STATIC_ATOMIC_INIT(0);

}  // namespace iobuf

size_t IOBuf::block_count() {
    return iobuf::g_nblock.load(sgxbutil::memory_order_relaxed);
}

size_t IOBuf::block_memory() {
    return iobuf::g_blockmem.load(sgxbutil::memory_order_relaxed);
}

size_t IOBuf::new_bigview_count() {
    return iobuf::g_newbigview.load(sgxbutil::memory_order_relaxed);
}
...
struct IOBuf::Block {
    ...
    Block(char* data_in, uint32_t data_size)
        ...
        , portal_next(NULL)
        , data(data_in) {
        iobuf::g_nblock.fetch_add(1, sgxbutil::memory_order_relaxed);
...
// #endif
    ...
    void inc_ref() {
        check_abi();
        nshared.fetch_add(1, sgxbutil::memory_order_relaxed);
    }
        
    void dec_ref() {
        check_abi();
        if (nshared.fetch_sub(1, sgxbutil::memory_order_release) == 1) {
            sgxbutil::atomic_thread_fence(sgxbutil::memory_order_acquire);
            if (!flags) {
                iobuf::g_nblock.fetch_sub(1, sgxbutil::memory_order_relaxed);
    ...
    int ref_count() const {
        return nshared.load(sgxbutil::memory_order_relaxed);
    }
...
static sgxbutil::static_atomic<size_t> g_num_hit_tls_threshold = BUTIL_STATIC_ATOMIC_INIT(0);
...
inline IOBuf::Block* share_tls_block() {
    ...
    if (b) {
        ...
        sgxbutil::thread_atexit(remove_tls_block_chain);
    }
    if (!new_block) {
...
inline void release_tls_block(IOBuf::Block *b) {
    ...
    if (b->full()) {
        ...
        b->dec_ref();
        g_num_hit_tls_threshold.fetch_add(1, sgxbutil::memory_order_relaxed);
    } else {
...
void release_tls_block_chain(IOBuf::Block* b) {
    ...
    if (tls_data.num_blocks >= MAX_BLOCKS_PER_THREAD) {
        ...
        g_num_hit_tls_threshold.fetch_add(n, sgxbutil::memory_order_relaxed);
        return;
    }
...
inline IOBuf::BlockRef* acquire_blockref_array(size_t cap) {
    iobuf::g_newbigview.fetch_add(1, sgxbutil::memory_order_relaxed);
...
size_t IOBuf::block_count_hit_tls_threshold() {
    return iobuf::g_num_hit_tls_threshold.load(sgxbutil::memory_order_relaxed);
}
...
ssize_t IOBuf::pcut_into_file_descriptor(int fd, off_t offset, size_t size_hint) {
    ...
    if (offset >= 0) {
        ...
        nw = (iobuf::get_writev_func())(fd, vec, nvec);
...
ssize_t IOBuf::pcut_multiple_into_file_descriptor(
    ...
    if (offset >= 0) {
        ...
        // VLOG(0) << "Func: " << __FUNCTION__ << " Calling writev";
        // nw = ::writev(fd, vec, nvec);
        nw = (iobuf::get_writev_func())(fd, vec, nvec);
    }
    if (nw <= 0) {
...
void IOBuf::append(const Movable& movable_other) {
    if (empty()) {
        ...
        sgxbutil::IOBuf& other = movable_other.value();
        const size_t nref = other._ref_num();
        for (size_t i = 0; i < nref; ++i) {
...
bool IOBuf::equals(const sgxbutil::StringPiece& s) const {
    if (size() != s.size()) {
        return false;
    }
    const size_t nref = _ref_num();
...
bool IOBuf::equals(const sgxbutil::IOBuf& other) const {
...
ssize_t IOPortal::append_from_SSL_channel(
    ...
    do {
        ...
        // VLOG(79) << "Func: " << __FUNCTION__ << " read_len = " << read_len;
...
IOBufCutter::IOBufCutter(sgxbutil::IOBuf* buf)
    : _data(NULL)
    , _data_end(NULL)
    , _block(NULL)
    , _buf(buf) {
...
size_t IOBufCutter::cutn(sgxbutil::IOBuf* out, size_t n) {
    if (n == 0) {
        return 0;
    }
...
IOBufAsSnappySink::IOBufAsSnappySink(sgxbutil::IOBuf& buf)
...
char* IOBufAsSnappySink::GetAppendBuffer(size_t length, char* scratch) {
    // TODO: sgxbutil::IOBuf supports dynamic sized blocks.
...
size_t IOBufBytesIterator::append_and_forward(sgxbutil::IOBuf* buf, size_t n) {
    size_t nc = 0;
...
//- TODO: May not be used in sgx-braft
// void* fast_memcpy(void *__restrict dest, const void *__restrict src, size_t n) {
//     return sgxbutil::iobuf::cp(dest, src, n);
// }
