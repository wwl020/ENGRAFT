...
#ifndef SGX_BUTIL_OBJECT_POOL_INL_H
#define SGX_BUTIL_OBJECT_POOL_INL_H
...
#include "sgxbutil/thread_local.h"           // BAIDU_THREAD_LOCAL
#include <vector>
...
public:
    ...
    struct BlockGroup {
        ...
        sgxbutil::atomic<Block*> blocks[OP_GROUP_NBLOCK];
        ...
        BlockGroup() : nblock(0) {
            ...
            memset(blocks, 0, sizeof(sgxbutil::atomic<Block*>) * OP_GROUP_NBLOCK);
        }
    };
...
#undef BAIDU_OBJECT_POOL_GET
    ...
    static inline ObjectPool* singleton() {
        ObjectPool* p = _singleton.load(sgxbutil::memory_order_consume);
        if (p) {
            return p;
        }
...
private:
    ...
    static sgxbutil::static_atomic<ObjectPool*> _singleton;
    ...
    static sgxbutil::static_atomic<BlockGroup*> _block_groups[OP_MAX_BLOCK_NGROUP];
...
template <typename T>
sgxbutil::static_atomic<ObjectPool<T>*> ObjectPool<T>::_singleton = BUTIL_STATIC_ATOMIC_INIT(NULL);
...
template <typename T>
sgxbutil::static_atomic<size_t> ObjectPool<T>::_ngroup = BUTIL_STATIC_ATOMIC_INIT(0);
...
template <typename T>
sgxbutil::static_atomic<typename ObjectPool<T>::BlockGroup*>
ObjectPool<T>::_block_groups[OP_MAX_BLOCK_NGROUP] = {};

#ifdef BUTIL_OBJECT_POOL_NEED_FREE_ITEM_NUM
template <typename T>
sgxbutil::static_atomic<size_t> ObjectPool<T>::_global_nfree = BUTIL_STATIC_ATOMIC_INIT(0);
#endif
...
#endif  // SGX_BUTIL_OBJECT_POOL_INL_H
