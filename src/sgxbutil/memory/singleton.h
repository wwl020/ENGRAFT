...
#ifndef SGX_BUTIL_MEMORY_SINGLETON_H_
#define SGX_BUTIL_MEMORY_SINGLETON_H_

#include "sgxbutil/at_exit.h"
...
struct StaticMemorySingletonTraits {
  ...
  static Type* New() {
    ...
    if (sgxbutil::subtle::NoBarrier_AtomicExchange(&dead_, 1))
      return NULL;
  ...
  static void Resurrect() {
    sgxbutil::subtle::NoBarrier_Store(&dead_, 0);
  }

 private:
  static sgxbutil::AlignedMemory<sizeof(Type), ALIGNOF(Type)> buffer_;
  // Signal the object was already deleted, so it is not revived.
  static sgxbutil::subtle::Atomic32 dead_;
};

template <typename Type> sgxbutil::AlignedMemory<sizeof(Type), ALIGNOF(Type)>
    StaticMemorySingletonTraits<Type>::buffer_;
template <typename Type> sgxbutil::subtle::Atomic32
...
// process exit. More precisely it uses sgxbutil::AtExitManager which requires an
...
class Singleton {
 private:
  ...
  static Type* get() {
    ...
    sgxbutil::subtle::AtomicWord value = sgxbutil::subtle::Acquire_Load(&instance_);
    if (value != 0 && value != sgxbutil::internal::kBeingCreatedMarker) {
    ...
    if (sgxbutil::subtle::Acquire_CompareAndSwap(
          &instance_, 0, sgxbutil::internal::kBeingCreatedMarker) == 0) {
      ...
      sgxbutil::subtle::Release_Store(
          &instance_, reinterpret_cast<sgxbutil::subtle::AtomicWord>(newval));

      if (newval != NULL && Traits::kRegisterAtExit)
        sgxbutil::AtExitManager::RegisterCallback(OnExit, NULL);

      return newval;
    }

    // We hit a race. Wait for the other thread to complete it.
    value = sgxbutil::internal::WaitForInstance(&instance_);
  ...
  static void OnExit(void* /*unused*/) {
    ...
    Traits::Delete(
        reinterpret_cast<Type*>(sgxbutil::subtle::NoBarrier_Load(&instance_)));
    instance_ = 0;
  }
  static sgxbutil::subtle::AtomicWord instance_;
};

template <typename Type, typename Traits, typename DifferentiatingType>
sgxbutil::subtle::AtomicWord Singleton<Type, Traits, DifferentiatingType>::
    instance_ = 0;

#endif  // SGX_BUTIL_MEMORY_SINGLETON_H_
