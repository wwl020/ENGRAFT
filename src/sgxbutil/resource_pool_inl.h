...
#ifndef SGXBUTIL_RESOURCE_POOL_INL_H
#define SGXBUTIL_RESOURCE_POOL_INL_H

#include <iostream>                      // std::ostream
#include <pthread.h>                     // pthread_mutex_t
#include <algorithm>                     // std::max, std::min
#include "sgxbutil/atomicops.h"              // sgxbutil::atomic
#include "sgxbutil/macros.h"                 // BAIDU_CACHELINE_ALIGNMENT
#include "sgxbutil/scoped_lock.h"            // BAIDU_SCOPED_LOCK
#include "sgxbutil/thread_local.h"           // thread_atexit
#include <vector>

#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_ADD1                \
    (_global_nfree.fetch_add(1, sgxbutil::memory_order_relaxed))
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_SUB1                \
    (_global_nfree.fetch_sub(1, sgxbutil::memory_order_relaxed))
#else
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_ADD1
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_SUB1
#endif
...
public:
    ...
    struct BlockGroup {
        sgxbutil::atomic<size_t> nblock;
        sgxbutil::atomic<Block*> blocks[RP_GROUP_NBLOCK];
        ...
        BlockGroup() : nblock(0) {
            ...
            memset(blocks, 0, sizeof(sgxbutil::atomic<Block*>) * RP_GROUP_NBLOCK);
        }
    };
...
#undef BAIDU_RESOURCE_POOL_GET
    ...
    static inline T* unsafe_address_resource(ResourceId<T> id) {
        ...
        return (T*)(_block_groups[(block_index >> RP_GROUP_NBLOCK_NBIT)]
                    .load(sgxbutil::memory_order_consume)
                    ->blocks[(block_index & (RP_GROUP_NBLOCK - 1))]
                    .load(sgxbutil::memory_order_consume)->items) +
    ...
    static inline T* address_resource(ResourceId<T> id) {
        ...
        if (__builtin_expect(group_index < RP_MAX_BLOCK_NGROUP, 1)) {
            BlockGroup* bg =
                _block_groups[group_index].load(sgxbutil::memory_order_consume);
    ...
    void clear_resources() {
        ...
        if (lp) {
            _local_pool = NULL;
            sgxbutil::thread_atexit_cancel(LocalPool::delete_local_pool, lp);
            delete lp;
        }
    }
    ...
    ResourcePoolInfo describe_resources() const {
        ...
        info.local_pool_num = _nlocal.load(sgxbutil::memory_order_relaxed);
        info.block_group_num = _ngroup.load(sgxbutil::memory_order_acquire);
        info.block_num = 0;
        info.item_num = 0;
        info.free_chunk_item_num = free_chunk_nitem();
        info.block_item_num = BLOCK_NITEM;
#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
        info.free_item_num = _global_nfree.load(sgxbutil::memory_order_relaxed);
#endif

        for (size_t i = 0; i < info.block_group_num; ++i) {
            BlockGroup* bg = _block_groups[i].load(sgxbutil::memory_order_consume);
            if (NULL == bg) {
                break;
            }
            size_t nblock = std::min(bg->nblock.load(sgxbutil::memory_order_relaxed),
                                     RP_GROUP_NBLOCK);
            info.block_num += nblock;
            for (size_t j = 0; j < nblock; ++j) {
                Block* b = bg->blocks[j].load(sgxbutil::memory_order_consume);
                if (NULL != b) {
    ...
    static inline ResourcePool* singleton() {
        ResourcePool* p = _singleton.load(sgxbutil::memory_order_consume);
        if (p) {
            return p;
        }
        pthread_mutex_lock(&_singleton_mutex);
        p = _singleton.load(sgxbutil::memory_order_consume);
        if (!p) {
            p = new ResourcePool();
            _singleton.store(p, sgxbutil::memory_order_release);
        } 
...
private:
    ...
    static Block* add_block(size_t* index) {
        ...
        size_t ngroup;
        do {
            ngroup = _ngroup.load(sgxbutil::memory_order_acquire);
            if (ngroup >= 1) {
                BlockGroup* const g =
                    _block_groups[ngroup - 1].load(sgxbutil::memory_order_consume);
                const size_t block_index =
                    g->nblock.fetch_add(1, sgxbutil::memory_order_relaxed);
                if (block_index < RP_GROUP_NBLOCK) {
                    g->blocks[block_index].store(
                        new_block, sgxbutil::memory_order_release);
                    *index = (ngroup - 1) * RP_GROUP_NBLOCK + block_index;
                    return new_block;
                }
                g->nblock.fetch_sub(1, sgxbutil::memory_order_relaxed);
            }
    ...
    static bool add_block_group(size_t old_ngroup) {
        ...
        const size_t ngroup = _ngroup.load(sgxbutil::memory_order_acquire);
        ...
        if (ngroup < RP_MAX_BLOCK_NGROUP) {
            ...
            if (NULL != bg) {
                ...
                // threads.
                _block_groups[ngroup].store(bg, sgxbutil::memory_order_release);
                _ngroup.store(ngroup + 1, sgxbutil::memory_order_release);
            }
        }
        return bg != NULL;
    }
    ...
    inline LocalPool* get_or_new_local_pool() {
        ...
        _local_pool = lp;
        sgxbutil::thread_atexit(LocalPool::delete_local_pool, lp);
        _nlocal.fetch_add(1, sgxbutil::memory_order_relaxed);
        return lp;
    }
    ...
    void clear_from_destructor_of_local_pool() {
        ...
        if (_nlocal.fetch_sub(1, sgxbutil::memory_order_relaxed) != 1) {
            return;
        }
        ...
        if (_nlocal.load(sgxbutil::memory_order_relaxed) != 0) {
            return;
        }
        ...
        // Delete all memory
        const size_t ngroup = _ngroup.exchange(0, sgxbutil::memory_order_relaxed);
        for (size_t i = 0; i < ngroup; ++i) {
            BlockGroup* bg = _block_groups[i].load(sgxbutil::memory_order_relaxed);
            if (NULL == bg) {
                break;
            }
            size_t nblock = std::min(bg->nblock.load(sgxbutil::memory_order_relaxed),
                                     RP_GROUP_NBLOCK);
            for (size_t j = 0; j < nblock; ++j) {
                Block* b = bg->blocks[j].load(sgxbutil::memory_order_relaxed);
                if (NULL == b) {
                    continue;
                }
...
private:
    ...
    static sgxbutil::static_atomic<ResourcePool*> _singleton;
    static pthread_mutex_t _singleton_mutex;
    static BAIDU_THREAD_LOCAL LocalPool* _local_pool;
    static sgxbutil::static_atomic<long> _nlocal;
    static sgxbutil::static_atomic<size_t> _ngroup;
    static pthread_mutex_t _block_group_mutex;
    static pthread_mutex_t _change_thread_mutex;
    static sgxbutil::static_atomic<BlockGroup*> _block_groups[RP_MAX_BLOCK_NGROUP];

    std::vector<DynamicFreeChunk*> _free_chunks;
    pthread_mutex_t _free_chunks_mutex;

#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
    static sgxbutil::static_atomic<size_t> _global_nfree;
#endif
};
...
template <typename T>
sgxbutil::static_atomic<ResourcePool<T>*> ResourcePool<T>::_singleton =
...
template <typename T>
sgxbutil::static_atomic<long> ResourcePool<T>::_nlocal = BUTIL_STATIC_ATOMIC_INIT(0);

template <typename T>
sgxbutil::static_atomic<size_t> ResourcePool<T>::_ngroup = BUTIL_STATIC_ATOMIC_INIT(0);
...
template <typename T>
sgxbutil::static_atomic<typename ResourcePool<T>::BlockGroup*>
ResourcePool<T>::_block_groups[RP_MAX_BLOCK_NGROUP] = {};

#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
template <typename T>
sgxbutil::static_atomic<size_t> ResourcePool<T>::_global_nfree = BUTIL_STATIC_ATOMIC_INIT(0);
#endif
...
#endif  // SGXBUTIL_RESOURCE_POOL_INL_H
