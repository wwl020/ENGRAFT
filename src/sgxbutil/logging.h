...
#include "sgxbutil/atomicops.h" // Used by LOG_EVERY_N, LOG_FIRST_N etc
#include "sgxbutil/time.h"      // gettimeofday_us()

#include <inttypes.h>
#include "google/gflags/gflags_declare.h"
...
public:
    ...
    virtual bool OnLogMessage(int severity, const char* file, int line,
                              const sgxbutil::StringPiece& log_content) = 0;
private:
...
public:
    bool OnLogMessage(int severity, const char* file, int line,
                 const sgxbutil::StringPiece& log_content) override;
private:
    sgxbutil::Lock _lock;
};
...
//     !(condition) ? ::sgxbutil::debug::BreakDebugger() : BAIDU_EAT_STREAM_PARAMS

// #define PCHECK(condition) CHECK(condition)

// #define BAIDU_CHECK_OP(name, op, val1, val2) CHECK((val1) op (val2))

#else //- defined(OFFICIAL_BUILD) && defined(NDEBUG)
...
public:
    ...
    sgxbutil::StringPiece content() const
    { return sgxbutil::StringPiece(pbase(), pptr() - pbase()); }
...
inline void LogAtLevel(int const log_level, const sgxbutil::StringPiece &msg) {
...
// operators.
//- TODO: Comment out the following line temporarily since we don't port butil::WideToUTF8 in butil/strings/utf_string_conversions.h.
...
#define BAIDU_LOG_IF_EVERY_N_IMPL(logifmacro, severity, condition, N)   \
    static ::sgxbutil::subtle::Atomic32 BAIDU_CONCAT(logeveryn_, __LINE__) = -1; \
    const static int BAIDU_CONCAT(logeveryn_sc_, __LINE__) = (N);       \
    const int BAIDU_CONCAT(logeveryn_c_, __LINE__) =                    \
        ::sgxbutil::subtle::NoBarrier_AtomicIncrement(&BAIDU_CONCAT(logeveryn_, __LINE__), 1); \
...
#define BAIDU_LOG_IF_FIRST_N_IMPL(logifmacro, severity, condition, N)   \
    static ::sgxbutil::subtle::Atomic32 BAIDU_CONCAT(logfstn_, __LINE__) = 0; \
    logifmacro(severity, (condition) && BAIDU_CONCAT(logfstn_, __LINE__) < N && \
               ::sgxbutil::subtle::NoBarrier_AtomicIncrement(&BAIDU_CONCAT(logfstn_, __LINE__), 1) <= N)

// Helper macro included by all *_EVERY_SECOND macros.
#define BAIDU_LOG_IF_EVERY_SECOND_IMPL(logifmacro, severity, condition) \
    static ::sgxbutil::subtle::Atomic64 BAIDU_CONCAT(logeverys_, __LINE__) = 0; \
    const int64_t BAIDU_CONCAT(logeverys_ts_, __LINE__) = ::sgxbutil::gettimeofday_us(); \
    ...
    logifmacro(severity, (condition) && BAIDU_CONCAT(logeverys_ts_, __LINE__) >= \
               ...
               ::sgxbutil::subtle::NoBarrier_CompareAndSwap(                \
