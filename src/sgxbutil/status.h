...
#ifndef SGX_BUTIL_STATUS_H
#define SGX_BUTIL_STATUS_H

//- TODO: OE don't support <stdarg.h> when using cpp, so use <cstdarg>
//- The above statement seems not true.
...
public:
    ...
    Status(int code, const sgxbutil::StringPiece& error_msg) : _state(NULL) {
    ...
    int set_error(int code, const char* error_format, ...)
        __attribute__ ((__format__ (__printf__, 3, 4)));
    int set_error(int code, const sgxbutil::StringPiece& error_msg);
    ...
    sgxbutil::StringPiece error_data() const {
        return (_state == NULL ? sgxbutil::StringPiece("OK", 2) 
                : sgxbutil::StringPiece(_state->message, _state->size));
    }
    std::string error_str() const;

    void swap(sgxbutil::Status& other) { std::swap(_state, other._state); }
...
inline void Status::operator=(const Status& s) {
    ...
    if (s._state == NULL) {
        ...
        set_error(s._state->code,
                  sgxbutil::StringPiece(s._state->message, s._state->size));
    }
}
