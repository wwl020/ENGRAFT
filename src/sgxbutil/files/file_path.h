...
// through sgxbutil::i18n::WrapPathWithLTRFormatting() before displaying it in the
// RTL UI.
//
...
#ifndef SGX_BUTIL_FILES_FILE_PATH_H_
#define SGX_BUTIL_FILES_FILE_PATH_H_
...
class BUTIL_EXPORT FilePath {
 public:
  ...
  // long "extensions" to logic like sgxbutil::GetUniquePathNumber().
  ...
  FilePath InsertBeforeExtensionASCII(
      const sgxbutil::StringPiece& suffix) const WARN_UNUSED_RESULT;
  ...
  FilePath AppendASCII(const sgxbutil::StringPiece& component)
  ...
  // std::string AsUTF8Unsafe() const; //- TODO: We don't want to include 
  //- butil/strings/sys_string_conversions.h, which is complex to port 
  ...
  // static FilePath FromUTF8Unsafe(const std::string& utf8); //- TODO: We don't want to include butil/strings/sys_string_conversions.h, which is complex to port
...
BUTIL_EXPORT extern void PrintTo(const sgxbutil::FilePath& path, std::ostream* out);
...
template<>
struct hash<sgxbutil::FilePath> {
  size_t operator()(const sgxbutil::FilePath& f) const {
    return hash<sgxbutil::FilePath::StringType>()(f.value());
  }
};
...
#endif  // SGX_BUTIL_FILES_FILE_PATH_H_
