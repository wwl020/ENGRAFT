...
#ifndef SGX_BUTIL_DOUBLY_BUFFERED_DATA_H
#define SGX_BUTIL_DOUBLY_BUFFERED_DATA_H

#include <vector>                                       // std::vector
#include <pthread.h>
#include "sgxbutil/scoped_lock.h"
#include "sgxbutil/thread_local.h"
// #include "sgxbutil/logging.h"
#include "sgxbutil/macros.h"
#include "sgxbutil/type_traits.h"
//- TODO: Is berror needed here?
#include "sgxbutil/errno.h"
#include "sgxbutil/atomicops.h"
//- Simply use memory header to use std::unique_ptr in c++ 11
...
private:
    ...
    { return _data + _index.load(sgxbutil::memory_order_acquire); }
    Wrapper* AddWrapper();
...
DoublyBufferedData<T, TLS>::DoublyBufferedData()
    ...
    const int rc = pthread_key_create(&_wrapper_key,
                                      sgxbutil::delete_object<Wrapper>);
    if (rc != 0) {
...
size_t DoublyBufferedData<T, TLS>::Modify(Fn& fn) {
    ...
    int bg_index = !_index.load(sgxbutil::memory_order_relaxed);
    ...
    _index.store(bg_index, sgxbutil::memory_order_release);
    bg_index = !bg_index;
    ...
    // CHECK_EQ(ret2, ret) << "index=" << _index.load(sgxbutil::memory_order_relaxed);
    return ret2;
}
...
#endif  // SGX_BUTIL_DOUBLY_BUFFERED_DATA_H
