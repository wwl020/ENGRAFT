...
#ifndef SGX_BUTIL_FLAT_MAP_H
#define SGX_BUTIL_FLAT_MAP_H
...
#include "sgxbutil/find_cstr.h"
#include "sgxbutil/single_threaded_pool.h"            // SingleThreadedPool
#include "sgxbutil/containers/hash_tables.h"          // hash<>
#include "sgxbutil/bit_array.h"                       // bit_array_*
...
struct DefaultHasher<std::string> {
    std::size_t operator()(const sgxbutil::StringPiece& s) const {
        std::size_t result = 0;
...
struct DefaultEqualTo<std::string> {
    ...
    { return s1 == s2; }
    bool operator()(const std::string& s1, const sgxbutil::StringPiece& s2) const
    { return s1 == s2; }
...
const _T* find_cstr(const FlatMap<std::string, _T, _Hash, _Equal, _Sparse>& m,
    ...
    return m.seek(sgxbutil::StringPiece(key, length));
}
...
#include "sgxbutil/containers/flat_map_inl.h"

#endif  //SGX_BUTIL_FLAT_MAP_H
