...
#include "sgxbutil/atomicops.h"     // sgxbutil::atomic<int>
...
void CountdownEvent::signal(int sig) {
    ...
    const int prev = ((sgxbutil::atomic<int>*)_butex)
        ->fetch_sub(sig, sgxbutil::memory_order_release);
...
int CountdownEvent::wait() {
    ...
    for (;;) {
        const int seen_counter = 
            ((sgxbutil::atomic<int>*)_butex)->load(sgxbutil::memory_order_acquire);
        if (seen_counter <= 0) {
            return 0;
        }
...
void CountdownEvent::add_count(int v) {
    ...
    ((sgxbutil::atomic<int>*)_butex)->fetch_add(v, sgxbutil::memory_order_release);
}
...
void CountdownEvent::reset(int v) {
    ...
    const int prev_counter =
            ((sgxbutil::atomic<int>*)_butex)
                ->exchange(v, sgxbutil::memory_order_release);
