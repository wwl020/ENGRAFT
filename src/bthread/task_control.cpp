...
#include "sgxbutil/fast_rand.h"             // BAIDU_SCOPED_LOCK
#include "sgxbutil/errno.h"                   // berror
...
int TaskControl::init(int concurrency) {
    ...
    // LOG(NOTICE) << "Func: " << __FUNCTION__ << " _concurrency = " << _concurrency;
    for (int i = 0; i < _concurrency; ++i) {
        // const int rc = pthread_create(&_workers[i], NULL, worker_thread, this);
        int rc = 0;
        ocall_pthread_general(&rc, &_workers[i], 
            generate_pthread_task(worker_thread, this), 0);
        if (rc) {
...
int TaskControl::add_workers(int num) {
    ...
    const int old_concurency = _concurrency.load(sgxbutil::memory_order_relaxed);
    for (int i = 0; i < num; ++i) {
        ...
        if (rc) {
            LOG(WARNING) << "Fail to create _workers[" << i + old_concurency
                         << "], " << berror(rc);
            _concurrency.fetch_sub(1, sgxbutil::memory_order_release);
            break;
        }
    }
    // Cannot fail
    _workers.resize(_concurrency.load(sgxbutil::memory_order_relaxed));
    return _concurrency.load(sgxbutil::memory_order_relaxed) - old_concurency;
}

TaskGroup* TaskControl::choose_one_group() {
    const size_t ngroup = _ngroup.load(sgxbutil::memory_order_acquire);
    if (ngroup != 0) {
        return _groups[sgxbutil::fast_rand_less_than(ngroup)];
    }
...
void TaskControl::stop_and_join() {
    ...
    // Stop workers
    {
        ...
        _stop = true;
        _ngroup.exchange(0, sgxbutil::memory_order_relaxed); 
    }
...
TaskControl::~TaskControl() {
    ...
    //       is extremely racy.
    //- Remove bvar
    // delete _pending_time.exchange(NULL, sgxbutil::memory_order_relaxed);
...
int TaskControl::_add_group(TaskGroup* g) {
    ...
    std::unique_lock<sgxbutil::Mutex> mu(_modify_group_mutex);
    if (_stop) {
        return -1;
    }
    size_t ngroup = _ngroup.load(sgxbutil::memory_order_relaxed);
...
int TaskControl::_destroy_group(TaskGroup* g) {
    ...
    {
        BAIDU_SCOPED_LOCK(_modify_group_mutex);
        const size_t ngroup = _ngroup.load(sgxbutil::memory_order_relaxed);
    ...
    if (erased) {
        get_global_timer_thread()->schedule(
            delete_task_group, g,
            sgxbutil::microseconds_from_now(FLAGS_task_group_delete_delay * 1000000L));
    }
    return 0;
}
...
bool TaskControl::steal_task(bthread_t* tid, size_t* seed, size_t offset) {
    ...
    const size_t ngroup = _ngroup.load(sgxbutil::memory_order_acquire/*1*/);
    if (0 == ngroup) {
        return false;
    }
...
void TaskControl::signal_task(int num_task) {
    ...
    int start_index = sgxbutil::fmix64(pthread_numeric_id()) % PARKING_LOT_NUM;
    ...
    if (num_task > 0 &&
        ...
        _concurrency.load(sgxbutil::memory_order_relaxed) < FLAGS_bthread_concurrency) {
        // TODO: Reduce this lock
        BAIDU_SCOPED_LOCK(g_task_control_mutex);
        if (_concurrency.load(sgxbutil::memory_order_acquire) < FLAGS_bthread_concurrency) {
            add_workers(1);
        }
    }
}
...
//     bvar::LatencyRecorder* pt = _pending_time.load(sgxbutil::memory_order_consume);
//     if (!pt) {
//         pt = new bvar::LatencyRecorder;
//         _pending_time.store(pt, sgxbutil::memory_order_release);
//         is_creator = true;
//     }
