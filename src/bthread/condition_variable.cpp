...
namespace bthread {
struct CondInternal {
    sgxbutil::atomic<bthread_mutex_t*> m;
    sgxbutil::atomic<int>* seq;
};
...
int bthread_cond_signal(bthread_cond_t* c) {
    ...
    // this point
    sgxbutil::atomic<int>* const saved_seq = ic->seq;
    saved_seq->fetch_add(1, sgxbutil::memory_order_release);
...
int bthread_cond_broadcast(bthread_cond_t* c) {
    bthread::CondInternal* ic = reinterpret_cast<bthread::CondInternal*>(c);
    bthread_mutex_t* m = ic->m.load(sgxbutil::memory_order_relaxed);
    sgxbutil::atomic<int>* const saved_seq = ic->seq;
    if (!m) {
        return 0;
    }
    ...
    ic->seq->fetch_add(1, sgxbutil::memory_order_release);
...
int bthread_cond_wait(bthread_cond_t* __restrict c,
    ...
    bthread::CondInternal* ic = reinterpret_cast<bthread::CondInternal*>(c);
    const int expected_seq = ic->seq->load(sgxbutil::memory_order_relaxed);
    if (ic->m.load(sgxbutil::memory_order_relaxed) != m) {
        // bind m to c
        bthread_mutex_t* expected_m = NULL;
        if (!ic->m.compare_exchange_strong(
                expected_m, m, sgxbutil::memory_order_relaxed)) {
            return EINVAL;
        }
    }
