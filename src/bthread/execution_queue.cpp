...
#include "sgxbutil/object_pool.h"           // sgxbutil::get_object
#include "sgxbutil/resource_pool.h"         // sgxbutil::get_resource
...
namespace /*anonymous*/ {
typedef sgxbutil::ResourceId<ExecutionQueueBase> slot_id_t;
...
//     return sgxbutil::get_leaky_singleton<ExecutionQueueVars>();
// }
...
void ExecutionQueueBase::start_execute(TaskNode* node) {
    ...
    if (node->high_priority) {
        ...
        _high_priority_tasks.fetch_add(1, sgxbutil::memory_order_relaxed);
    }
    TaskNode* const prev_head = _head.exchange(node, sgxbutil::memory_order_release);
    if (prev_head != NULL) {
        node->next = prev_head;
        return;
    }
    ...
    if (node->in_place) {
        ...
        // return if no more
        if (node->high_priority) {
            _high_priority_tasks.fetch_sub(niterated, sgxbutil::memory_order_relaxed);
        }
...
void* ExecutionQueueBase::_execute_tasks(void* arg) {
    ...
    for (;;) {
        ...
        int rc = 0;
        if (m->_high_priority_tasks.load(sgxbutil::memory_order_relaxed) > 0) {
            int nexecuted = 0;
            // Don't care the return value
            rc = m->_execute(head, true, &nexecuted);
            m->_high_priority_tasks.fetch_sub(
                    nexecuted, sgxbutil::memory_order_relaxed);
            if (nexecuted == 0) {
    ...
    if (destroy_queue) {
        CHECK(m->_head.load(sgxbutil::memory_order_relaxed) == NULL);
        CHECK(m->_stopped);
        ...
        m->_join_butex->fetch_add(2, sgxbutil::memory_order_release/*1*/);
        butex_wake_all(m->_join_butex);
        //- Remove bvar
        // vars->execq_count << -1;
        sgxbutil::return_resource(slot_of_id(m->_this_id));
    }
    //- Remove bvar
    // vars->execq_active_count << -1;
    return NULL;
...
void ExecutionQueueBase::return_task_node(TaskNode* node) {
    ...
    sgxbutil::return_object<TaskNode>(node);
    //- Remove bvar
...
void ExecutionQueueBase::_on_recycle() {
    // Push a closed tasks
    while (true) {
        TaskNode* node = sgxbutil::get_object<TaskNode>();
...
int ExecutionQueueBase::join(uint64_t id) {
    ...
    ExecutionQueueBase* const m = sgxbutil::address_resource(slot);
    if (m == NULL) {
    ...
    while (expected == m->_join_butex->load(sgxbutil::memory_order_acquire)) {
...
int ExecutionQueueBase::stop() {
    ...
    for (;;) {
        ...
        if (_versioned_ref.compare_exchange_strong(
            ...
            _stopped.store(true, sgxbutil::memory_order_release);
...
TaskNode* ExecutionQueueBase::allocate_node() {
    //- Remove bvar
    // get_execq_vars()->running_task_count << 1;
    return sgxbutil::get_object<TaskNode>();
}
...
int ExecutionQueueBase::create(uint64_t* id, const ExecutionQueueOptions* options,
    ...
    slot_id_t slot;
    ExecutionQueueBase* const m = sgxbutil::get_resource(&slot, Forbidden());
    if (BAIDU_LIKELY(m != NULL)) {
        ...
        CHECK(m->_head.load(sgxbutil::memory_order_relaxed) == NULL);
        CHECK_EQ(0, m->_high_priority_tasks.load(sgxbutil::memory_order_relaxed));
        ExecutionQueueOptions opt;
        if (options != NULL) {
            opt = *options;   
        }
        m->_options = opt;
        m->_stopped.store(false, sgxbutil::memory_order_relaxed);
        m->_this_id = make_id(
...
inline bool TaskIteratorBase::should_break_for_high_priority_tasks() {
    if (!_high_priority && 
            _q->_high_priority_tasks.load(sgxbutil::memory_order_relaxed) > 0) {
        _should_break = true;
        return true;
    }
    return false;
