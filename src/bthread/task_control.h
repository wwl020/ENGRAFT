...
#include "sgxbutil/resource_pool.h"                 // ResourcePool
...
public:
    ...
    { return _concurrency.load(sgxbutil::memory_order_acquire); }
...
private:
    ...
    sgxbutil::atomic<size_t> _ngroup;
    TaskGroup** _groups;
    sgxbutil::Mutex _modify_group_mutex;

    bool _stop;
    sgxbutil::atomic<int> _concurrency;
    std::vector<pthread_t> _workers;

    //- Remove bvar
    // bvar::Adder<int64_t> _nworkers;
    sgxbutil::Mutex _pending_time_mutex;
    //- Remove bvar
    // sgxbutil::atomic<bvar::LatencyRecorder*> _pending_time;
