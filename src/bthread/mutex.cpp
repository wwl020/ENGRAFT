...
//- To remove bvar library in this file, g_cp (global contention profile) variable 
//- is always NULL. Thus, some variables/branches can be eliminated.
...
//         sgxbutil::MurmurHash3_x86_32(stack, sizeof(void*) * nframes, seed, &code);
//         return code;
//     }
// };
...
// public:
//     typedef sgxbutil::FlatMap<SampledContention*, SampledContention*,
...
//     sgxbutil::IOBuf _disk_buf;  // temp buf before saving the file.
...
//         sgxbutil::IOPortal mem_maps;
//         const sgxbutil::fd_guard fd(open("/proc/self/maps", O_RDONLY));
//         if (fd >= 0) {
//             while (true) {
...
//     sgxbutil::FilePath path(_filename);
//     sgxbutil::FilePath dir = path.DirName();
//     if (!sgxbutil::CreateDirectoryAndGetError(dir, &error)) {
...
//         flag = O_TRUNC;
//     }
//     sgxbutil::fd_guard fd(open(_filename.c_str(), O_WRONLY|O_CREAT|flag, 0666));
//     if (fd < 0) {
...
//     sgxbutil::static_atomic<uint64_t> versioned_mutex;
...
//     sgxbutil::return_object(this);
// }

// // Remember the conflict hashes for troubleshooting, should be 0 at most of time.
// static sgxbutil::static_atomic<int64_t> g_nconflicthash = BUTIL_STATIC_ATOMIC_INIT(0);
// static int64_t get_nconflicthash(void*) {
//     return g_nconflicthash.load(sgxbutil::memory_order_relaxed);
// }
...
//   #1  0x00000000004b4df7 in sgxbutil::internal::SpinLockDelay(int volatile*, int, int) ()
...
inline uint64_t hash_mutex_ptr(const pthread_mutex_t* m) {
    return sgxbutil::fmix64((uint64_t)m);
}
...
//     sgxbutil::static_atomic<uint64_t>& m = entry.versioned_mutex;
//     uint64_t expected = m.load(sgxbutil::memory_order_relaxed);
...
//                 expected, desired, sgxbutil::memory_order_acquire)) {
//             return &entry.csite;
//         }
//     }
//     g_nconflicthash.fetch_add(1, sgxbutil::memory_order_relaxed);
//     return NULL;
// }
...
//     MutexMapEntry& entry = g_mutex_map[hash_mutex_ptr(mutex) & (MUTEX_MAP_SIZE - 1)];
//     sgxbutil::static_atomic<uint64_t>& m = entry.versioned_mutex;
//     if ((m.load(sgxbutil::memory_order_relaxed) & ((((uint64_t)1) << PTR_BITS) - 1))
//         != (uint64_t)mutex) {
...
//     m.store(0, sgxbutil::memory_order_release);
//     return true;
// }
...
//     SampledContention* sc = sgxbutil::get_object<SampledContention>();
...
//     const int64_t start_ns = sgxbutil::cpuwide_time_ns();
...
//             }
//         }
//         csite->duration_ns = sgxbutil::cpuwide_time_ns() - start_ns;
...
//                 unlock_start_ns = sgxbutil::cpuwide_time_ns();
//             }
...
//     // [Outside lock]
//     if (unlock_start_ns) {
//         const int64_t unlock_end_ns = sgxbutil::cpuwide_time_ns();
...
struct MutexInternal {
    sgxbutil::static_atomic<unsigned char> locked;
    sgxbutil::static_atomic<unsigned char> contended;
...
inline int mutex_lock_contended(bthread_mutex_t* m) {
    sgxbutil::atomic<unsigned>* whole = (sgxbutil::atomic<unsigned>*)m->butex;
...
int FastPthreadMutex::lock_contended() {
    sgxbutil::atomic<unsigned>* whole = (sgxbutil::atomic<unsigned>*)&_futex;
    while (whole->exchange(BTHREAD_MUTEX_CONTENDED) & BTHREAD_MUTEX_LOCKED) {
...
void FastPthreadMutex::lock() {
    ...
    if (split->locked.exchange(1, sgxbutil::memory_order_acquire)) {
        (void)lock_contended();
    }
}
...
bool FastPthreadMutex::try_lock() {
    bthread::MutexInternal* split = (bthread::MutexInternal*)&_futex;
    return !split->locked.exchange(1, sgxbutil::memory_order_acquire);
}

void FastPthreadMutex::unlock() {
    sgxbutil::atomic<unsigned>* whole = (sgxbutil::atomic<unsigned>*)&_futex;
    const unsigned prev = whole->exchange(0, sgxbutil::memory_order_release);
...
int bthread_mutex_lock(bthread_mutex_t* m) {
    ...
    // if (!rc) { // Inside lock
    //     m->csite.duration_ns = sgxbutil::cpuwide_time_ns() - start_ns;
...
int bthread_mutex_timedlock(bthread_mutex_t* __restrict m,
    ...
    //     const int64_t end_ns = sgxbutil::cpuwide_time_ns();
...
int bthread_mutex_unlock(bthread_mutex_t* m) {
    ...
    // const int64_t unlock_start_ns = sgxbutil::cpuwide_time_ns();
    bthread::butex_wake(whole);
    // const int64_t unlock_end_ns = sgxbutil::cpuwide_time_ns();
    // saved_csite.duration_ns += unlock_end_ns - unlock_start_ns;
...
//- The reason why pthread_mutex_lock/unlock are overwritten here is that bthread
//- hopes to profile contention. If profile is off, we can safely comment out the
//- following overwritten functions.
